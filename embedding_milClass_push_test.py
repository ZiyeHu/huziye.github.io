...
flags.DEFINE_integer('restore_iter', 29999, 'iteration to load model (-1 for latest model)')
...
flags.DEFINE_integer('begin_restore_iter', 21300, 'iteration to load model (-1 for latest model)')
    ...
    with graph.as_default():
        ...
        with tf.variable_scope('model', reuse=tf.AUTO_REUSE) as training_scope:
            ...
            def batch_metalearn(inp):
                ...
                semantic_loss =  1e3 * act_loss_eps * euclidean_loss_layer(semantic_outputb, compare_semantic_outputb, multiplier=loss_multiplier,use_l1=FLAGS.use_l1_l2_loss) +\
                                 tf.abs((FLAGS.margin - 1e3 *  act_loss_eps * euclidean_loss_layer(semantic_outputb, different_semantic_outputb, multiplier=loss_multiplier, use_l1=FLAGS.use_l1_l2_loss)))+ \
                                 tf.abs((FLAGS.margin - 1e3 *  act_loss_eps * euclidean_loss_layer(compare_semantic_outputb, different_semantic_outputb, multiplier=loss_multiplier, use_l1=FLAGS.use_l1_l2_loss)))


                # semantic_loss = tf.reduce_max(semantic_outputb) + tf.reduce_min(semantic_outputb)
                print('semantic_loss', semantic_loss.shape)


                local_lossb = 0.5 * (act_loss_eps * euclidean_loss_layer(compare_outputb, compare_action, multiplier=loss_multiplier,use_l1=FLAGS.use_l1_l2_loss) +

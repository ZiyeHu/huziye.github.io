            ...
            def batch_metalearn(inp):
                ...
                if FLAGS.compare_learn:
                    ...
                    with tf.variable_scope('select_model', reuse=tf.AUTO_REUSE) as select_scope:
                        ...
                        # Pre-update
                        if 'Training' in prefix:
                            local_outputa, final_eept_preda = forward(conv_out_size, inputb,tf.zeros_like(state_inputb), fast_weights,network_config=network_config)
                        else:
                            local_outputa, final_eept_preda = forward(conv_out_size, inputb,tf.zeros_like(state_inputb), fast_weights,is_training=False, network_config=network_config)
                        if FLAGS.learn_final_eept:
                            final_eept_lossa = euclidean_loss_layer(final_eept_preda, final_eepta,multiplier=loss_multiplier,use_l1=FLAGS.use_l1_l2_loss)
                        else:
                            final_eept_lossa = tf.constant(0.0)

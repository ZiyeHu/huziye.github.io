...
flags.DEFINE_integer('num_filters', 15,
        ...
        with tf.variable_scope('model', reuse=tf.AUTO_REUSE) as training_scope:
            ...
            def batch_metalearn(inp):
                ...
                if 'Training' in prefix:
                    ...
                    compare_learn_outputb, compare_learn_final_eept_predb = forward(conv_out_size, inputa, tf.zeros_like(state_inputa),
                                                        compare_learn_fast_weights, network_config=network_config)
                else:
                    ...
                    compare_learn_outputb, compare_learn_final_eept_predb = forward(conv_out_size, inputa, tf.zeros_like(state_inputa),
                                                        compare_learn_fast_weights, is_training=False,
                                                        testing=testing,network_config=network_config)
                ...
                right_lossb = act_loss_eps * euclidean_loss_layer(compare_learn_outputb, tf.zeros_like(compare_learn_outputb), multiplier=loss_multiplier,use_l1=FLAGS.use_l1_l2_loss)


                if FLAGS.compare_learn:
                    # if 'Training' in prefix:
                    #     compare_learn_outputb, compare_learn_final_eept_predb = forward(conv_out_size, inputa, state_inputa, compare_learn_fast_weights,meta_testing=True, network_config=network_config)
                    #
                    # else:
                    #     compare_learn_outputb, compare_learn_final_eept_predb = forward(conv_out_size, inputa, state_inputa, compare_learn_fast_weights,meta_testing=True, is_training=False, testing=testing,network_config=network_config)
                    ...
                    with tf.variable_scope('select_model', reuse=tf.AUTO_REUSE) as select_scope:
                        ...
                        grads = tf.gradients(right_lossb, compare_learn_fast_weights.values())
                        gradients = dict(zip(compare_learn_fast_weights.keys(), grads))
                        # make fast gradient zero for weights with gradient None
                        for key in gradients.keys():
                            if gradients[key] is None:
                                gradients[key] = tf.zeros_like(compare_learn_fast_weights[key])
                        if FLAGS.stop_grad:
                            gradients = {key: tf.stop_gradient(gradients[key]) for key in gradients.keys()}
                        if FLAGS.clip:
                            clip_min = FLAGS.clip_min
                        ...
                        if FLAGS.pretrain_weight_path != 'N/A':
                            gradients['wc1'] = tf.zeros_like(gradients['wc1'])
                            gradients['bc1'] = tf.zeros_like(gradients['bc1'])
                        compare_learn_fast_weights = dict(zip(compare_learn_fast_weights.keys(),
                                                              [compare_learn_fast_weights[key] - step_size * gradients[key]
                                                               for key in compare_learn_fast_weights.keys()]))
                        ...
                        if 'Training' in prefix:
                            ...
                            compare_learn_merge_local_outputa, compare_learn_merge_final_eept_preda = forward(conv_out_size, inputa,
                                                                                  tf.zeros_like(state_inputa),
                                                                                  compare_learn_fast_weights,
                                                                                  network_config=network_config)
                            compare_learn_merge_local_lossa = act_loss_eps * euclidean_loss_layer(compare_learn_merge_local_outputa,
                                                                                    tf.zeros_like(compare_learn_merge_local_outputa),
                                                                                    multiplier=loss_multiplier,
                                                                                    use_l1=FLAGS.use_l1_l2_loss)
                        else:
                            ...
                            compare_learn_merge_local_outputa, compare_learn_merge_final_eept_preda = forward(
                                conv_out_size, inputa,
                                tf.zeros_like(state_inputa),
                                compare_learn_fast_weights,
                                is_training=False,
                                network_config=network_config)
                        ...
                        # Compute fast gradients
                        grads = tf.gradients(compare_learn_merge_local_lossa, compare_learn_fast_weights.values())
                        gradients = dict(zip(compare_learn_fast_weights.keys(), grads))
                        # make fast gradient zero for weights with gradient None
                        for key in gradients.keys():
                            if gradients[key] is None:
                        ...
                        compare_learn_fast_weights = dict(zip(compare_learn_fast_weights.keys(),
                                                      [compare_learn_fast_weights[key] - step_size * gradients[key] for key in
                                                       compare_learn_fast_weights.keys()]))
                        ...
                        if 'Training' in prefix:
                            ...
                            learn_outputb, _ = forward(conv_out_size, compare_input,compare_state_input,
                                                                                    merge_fast_weights,
                                                                                    meta_testing=True,
                                                                                    network_config=network_config)
                            left_lossb = act_loss_eps * euclidean_loss_layer(learn_outputb, compare_action,
                                                                              multiplier=loss_multiplier,
                                                                              use_l1=FLAGS.use_l1_l2_loss)
                        else:
                            ...
                            learn_outputb, _ = forward(conv_out_size, compare_input,compare_state_input,
                                                                                    merge_fast_weights,
                                                                                    meta_testing=True,
                                                                                    is_training=False, testing=testing,
                                                                                    network_config=network_config)
                            left_lossb = act_loss_eps * euclidean_loss_layer(learn_outputb, compare_action,
                                                                              multiplier=loss_multiplier,
                                                                              use_l1=FLAGS.use_l1_l2_loss)
                        ...
                        outputb=(compare_learn_outputb+learn_outputb)*0.5

...
# flags.DEFINE_string('log_dir', 'logs/sim_reach_temporal_conv_with_bicycle', 'directory for summaries and checkpoints.')
flags.DEFINE_string('log_dir', 'logs/sim_reach_temporal_conv', 'directory for summaries and checkpoints.')
...
flags.DEFINE_integer('begin_restore_iter', 29999, 'iteration to load model (-1 for latest model)')
    ...
    with graph.as_default():
        ...
        with tf.variable_scope('model', reuse=tf.AUTO_REUSE) as training_scope:
            ...
            def batch_metalearn(inp):
                ...
                inputa, inputb,compare_input, actiona, actionb,compare_action = inp
                ...
                if 'Training' in prefix:
                    ...
                    local_outputa, final_eept_preda = forward(conv_out_size, inputa, state_inputa, weights, meta_testing=True, network_config=network_config)
                else:
                    local_outputa, final_eept_preda = forward(conv_out_size, inputa, state_inputa, weights, meta_testing=True, is_training=False, network_config=network_config)
                if FLAGS.learn_final_eept:
                ...
                #     if 'Training' in prefix:
                #         compare_learn_local_outputa, compare_learn_final_eept_preda = forward(conv_out_size, compare_input, compare_state_input, compare_learn_weights,network_config=network_config)
                #     else:
                #         compare_learn_local_outputa, compare_learn_final_eept_preda = forward(conv_out_size, compare_input, compare_state_input, compare_learn_weights,is_training=False, network_config=network_config)
                #
                ...
                # fast_weights = dict(zip(weights.keys(), [weights[key] - step_size * step_size * step_size * gradients[key] for key in weights.keys()]))
                fast_weights = dict(zip(weights.keys(),
                                        [weights[key] - step_size * gradients[key] for key in
                                         weights.keys()]))

                # if FLAGS.compare_learn:
                #     # Compute fast gradients
                #     compare_learn_grads = tf.gradients(compare_learn_local_lossa, compare_learn_weights.values())
                #     compare_learn_gradients = dict(zip(compare_learn_weights.keys(), compare_learn_grads))
                #     # make fast gradient zero for weights with gradient None
                #     for key in compare_learn_gradients.keys():
                #         if compare_learn_gradients[key] is None:
                #             compare_learn_gradients[key] = tf.zeros_like(compare_learn_weights[key])
                #     if FLAGS.stop_grad:
                #         compare_learn_gradients = {key: tf.stop_gradient(compare_learn_gradients[key]) for key in compare_learn_gradients.keys()}
                #     if FLAGS.clip:
                #         clip_min = FLAGS.clip_min
                #         clip_max = FLAGS.clip_max
                #         for key in compare_learn_gradients.keys():
                #             compare_learn_gradients[key] = tf.clip_by_value(compare_learn_gradients[key], clip_min, clip_max)
                #     if FLAGS.pretrain_weight_path != 'N/A':
                #         compare_learn_gradients['wc1'] = tf.zeros_like(compare_learn_gradients['wc1'])
                #         compare_learn_gradients['bc1'] = tf.zeros_like(compare_learn_gradients['bc1'])
                #     compare_learn_fast_weights = dict(zip(compare_learn_weights.keys(), [compare_learn_weights[key] - step_size * compare_learn_gradients[key] for key in compare_learn_weights.keys()]))
                ...
                if 'Training' in prefix:
                    ...
                    outputb, final_eept_predb = forward(conv_out_size, inputb, tf.zeros_like(state_inputb),
                                                        fast_weights, network_config=network_config)
                else:
                    ...
                    outputb, final_eept_predb = forward(conv_out_size, inputb, tf.zeros_like(state_inputb),
                                                        fast_weights, is_training=False,
                                                        testing=testing, network_config=network_config)

                left_lossb = act_loss_eps * euclidean_loss_layer(outputb, tf.zeros_like(outputb), multiplier=loss_multiplier,use_l1=FLAGS.use_l1_l2_loss)
                # local_lossb = act_loss_eps * euclidean_loss_layer(outputb, actionb, multiplier=loss_multiplier,use_l1=FLAGS.use_l1_l2_loss)


                if FLAGS.compare_learn:
                #     if 'Training' in prefix:
                #         compare_learn_outputb, compare_learn_final_eept_predb = forward(conv_out_size, inputb, state_inputb, compare_learn_fast_weights,meta_testing=True, network_config=network_config)
                #
                #     else:
                #         compare_learn_outputb, compare_learn_final_eept_predb = forward(conv_out_size, inputb, state_inputb, compare_learn_fast_weights,meta_testing=True, is_training=False, testing=testing,network_config=network_config)
                #
                #     # mix_outputb = (outputb+compare_learn_outputb)*0.5
                #     left_lossb = act_loss_eps * euclidean_loss_layer(outputb, actionb, multiplier=loss_multiplier, use_l1=FLAGS.use_l1_l2_loss)
                #     right_lossb = act_loss_eps * euclidean_loss_layer(compare_learn_outputb, actionb, multiplier=loss_multiplier, use_l1=FLAGS.use_l1_l2_loss)
                    # mix_lossb = act_loss_eps * euclidean_loss_layer(mix_outputb, actionb,multiplier=loss_multiplier,use_l1=FLAGS.use_l1_l2_loss)

                    # total_left_lossb = tf.reduce_sum(left_lossb)
                    # total_right_lossb = tf.reduce_sum(right_lossb)
                    #
                    # left_lossb = tf.case({tf.greater(total_left_lossb,total_right_lossb): (lambda :left_lossb * 2.0)},default=lambda:left_lossb)
                    # right_lossb = tf.case({tf.greater(total_right_lossb, total_left_lossb): (lambda:right_lossb * 2.0)},default=lambda:right_lossb)


                    # outputb = (outputb + compare_learn_outputb) * 0.5



                    # mix_outputb = tf.concat([outputb, compare_learn_outputb], axis=-1)
                    # mix_outputb = norm(mix_outputb, norm_type=norm_type, id=100, activation_fn=tf.nn.relu)



                    with tf.variable_scope('select_model', reuse=tf.AUTO_REUSE) as select_scope:

                        grads = tf.gradients(left_lossb, fast_weights.values())
                        ...
                        if FLAGS.pretrain_weight_path != 'N/A':
                            ...
                            gradients['bc1'] = tf.zeros_like(gradients['bc1'])
                        # gradients_summ.append([gradients[key] for key in sorted_weight_keys])
                        merge_weights = dict(zip(fast_weights.keys(), [fast_weights[key] - step_size * gradients[key] for key in fast_weights.keys()]))

                        if 'Training' in prefix:
                            # merge_local_outputa, merge_final_eept_preda = forward(conv_out_size, inputa, tf.zeros_like(state_inputa), merge_weights,network_config=network_config)
                            # merge_local_lossa = act_loss_eps * euclidean_loss_layer(merge_local_outputa, tf.zeros_like(actiona),multiplier=loss_multiplier,use_l1=FLAGS.use_l1_l2_loss)
                            merge_local_outputa, merge_final_eept_preda = forward(conv_out_size, inputb,
                                                                                  tf.zeros_like(state_inputb),
                                                                                  merge_weights,
                                                                                  network_config=network_config)
                            merge_local_lossa = act_loss_eps * euclidean_loss_layer(merge_local_outputa,
                                                                                    tf.zeros_like(merge_local_outputa),
                                                                                    multiplier=loss_multiplier,
                                                                                    use_l1=FLAGS.use_l1_l2_loss)
                        else:
                            # merge_local_outputa, merge_final_eept_preda = forward(conv_out_size, inputa, tf.zeros_like(state_inputa), merge_weights,is_training=False, network_config=network_config)
                            # merge_local_lossa = act_loss_eps * euclidean_loss_layer(merge_local_outputa, tf.zeros_like(actiona),multiplier=loss_multiplier,use_l1=FLAGS.use_l1_l2_loss)
                            merge_local_outputa, merge_final_eept_preda = forward(conv_out_size, inputb,
                                                                                  tf.zeros_like(state_inputb),
                                                                                  merge_weights, is_training=False,
                                                                                  network_config=network_config)
                            merge_local_lossa = act_loss_eps * euclidean_loss_layer(merge_local_outputa,
                                                                                    tf.zeros_like(merge_local_outputa),
                                                                                    multiplier=loss_multiplier,
                        ...
                        # Compute fast gradients
                        grads = tf.gradients(merge_local_lossa, merge_weights.values())
                        gradients = dict(zip(merge_weights.keys(), grads))
                        # make fast gradient zero for weights with gradient None
                        for key in gradients.keys():
                            if gradients[key] is None:
                                gradients[key] = tf.zeros_like(merge_weights[key])
                        if FLAGS.stop_grad:
                            gradients = {key: tf.stop_gradient(gradients[key]) for key in gradients.keys()}
                        if FLAGS.clip:
                            clip_min = FLAGS.clip_min
                        ...
                        if FLAGS.pretrain_weight_path != 'N/A':
                            ...
                            gradients['bc1'] = tf.zeros_like(gradients['bc1'])
                        # gradients_summ.append([gradients[key] for key in sorted_weight_keys])
                        merge_fast_weights = dict(zip(merge_weights.keys(), [merge_weights[key] - step_size * gradients[key] for key in merge_weights.keys()]))

                        # Post-update
                        if 'Training' in prefix:
                            compare_learn_outputb, merge_final_eept_predb = forward(conv_out_size, compare_input, compare_state_input, merge_fast_weights,meta_testing=True, network_config=network_config)
                            ...
                            compare_learn_outputb, merge_final_eept_predb = forward(conv_out_size, compare_input, compare_state_input, merge_fast_weights,meta_testing=True, is_training=False, testing=testing,network_config=network_config)
                            right_lossb = act_loss_eps * euclidean_loss_layer(compare_learn_outputb, compare_action,multiplier=loss_multiplier,use_l1=FLAGS.use_l1_l2_loss)
        ...
        def train(graph, saver, sess, data_generator, log_dir, restore_itr=0):
            """
            Train the
            """
            PRINT_INTERVAL = 100
            ...
            for itr in training_range:
                ...
                # obsa_compare_input = train_compare_image_datas[:, :FLAGS.update_batch_size * FLAGS.T, :]
                # obsb_compare_input = train_compare_image_datas[:, FLAGS.update_batch_size * FLAGS.T:, :]
                ...
                # print('compare_statea_input',compare_statea_input.shape,'statea_input',statea_input.shape)
                ...
                # feed_dict = {global_obsa: obsa_compare_input,
                #              global_obsb: obsb_input,
                #              global_compare_obs:obsa_compare_input,
        ...
        def evaluate_vision_reach(env, graph, data_generator, sess, exp_string, record_gifs, log_dir):
            T = FLAGS.T
                ...
                for j in xrange(REACH_DEMO_CONDITIONS):
                    if j in data_generator.demos[i]['demoConditions']:
                        ...
                        for t in range(T):
                            ...
                            #     global_compare_obs: selected_demoO,
                            #     global_statea: selected_demoX.dot(scale) + bias,
                            #     global_compare_state: selected_demoX.dot(scale) + bias,
                            #     global_actiona: selected_demoU,
                            ...
                            feed_dict = {
                                ...
                                global_compare_obs: obs,
                                global_statea: state.dot(scale) + bias,
        ...
        def evaluate_rl_vision_reach(graph, data_generator, sess, exp_string, record_gifs, log_dir,iterations=10,learn_examples=150):
              ...
              for i in xrange(learn_examples):
                ...
                for j in xrange(REACH_DEMO_CONDITIONS):
                    if j in data_generator.demos[i]['demoConditions']:
                        ...
                        if rl_rewards>-2.5 and rl_rewards>total_reward:
                            feed_dict = {
                                global_obsb: selected_demoO,
                                global_stateb: selected_demoX.dot(scale) + bias,
                                global_obsa: rl_obs,
                                global_statea: rl_states,
                            ...
                            input_tensors = [self_learn_train_op, total_right_lossb]
                            results = sess.run(input_tensors, feed_dict=feed_dict)
        ...
        if FLAGS.train:
            ...
            train(graph, saver, sess, data_generator, log_dir, restore_itr=FLAGS.restore_iter)
            ...
            if FLAGS.begin_restore_iter!=FLAGS.end_restore_iter:
                ...
                while iter_index<=FLAGS.end_restore_iter:
                    ...
                    if 'reach' in FLAGS.experiment:
                        ...
                        evaluate_rl_vision_reach(graph, data_generator, sess, exp_string, FLAGS.record_gifs, log_dir)
                    iter_index+=100
            else:
                if FLAGS.restore_iter > 0:

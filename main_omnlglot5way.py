...
def train(model,saver, sess, exp_string, data_generator, resume_itr=0):
    ...
    for itr in range(resume_itr, FLAGS.pretrain_iterations + FLAGS.metatrain_iterations):
        ...
        # temp_input = model.inputb
        # temp_label = model.labelb
        #
        # model.inputb = model.inputa
        # model.labelb = model.labela
        # result = sess.run(input_tensors, feed_dict)
        #
        # model.inputb = temp_input
        # model.labelb = temp_label
        feed_tensors=[model.temp_inputa, model.temp_inputb,  model.temp_labela, model.temp_labelb]
        out=sess.run(feed_tensors, feed_dict)
        # print('out is',np.sum(out[0]))
        # # out = sess.run(feed_tensors, feed_dict)
        # print('out2 is', np.sum(out[0]))

        result = sess.run(input_tensors, feed_dict={model.inputa: out[0], model.inputb: out[1],  model.labela: out[2], model.labelb: out[3]})
        result = sess.run(input_tensors, feed_dict={model.inputa: out[0], model.inputb: out[1],  model.labela: out[2], model.labelb: out[3]})
...
def test(model, saver, sess, exp_string, data_generator, test_num_updates=None):
    ...
    for _ in range(NUM_TEST_POINTS):
        ...
        if model.classification:
            # print('test feed_dict',feed_dict)
...
def main():
    ...
    if FLAGS.datasource == 'miniimagenet' or FLAGS.datasource == 'omniglot':
        ...
        if FLAGS.train: # only construct training model if needed
            ...
            # previous_input_tensors = {'inputa': inputa, 'inputb': inputa, 'labela': labela, 'labelb': labela}
    ...
    if FLAGS.train or not tf_data_load:
        ...
        # model.construct_model(input_tensors=previous_input_tensors, prefix='metatrain_')

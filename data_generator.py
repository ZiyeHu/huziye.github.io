...
class DataGenerator(object):
    ...
    def make_compare_batch_tensor(self, network_config, restore_iter=0, train=True):
        ...
        if train:
            all_filenames = self.all_training_filenames
            all_compare_filenames = self.all_compare_training_filenames
            all_different_filenames = self.all_different_training_filenames
            print('all_training_filenames', len(self.all_training_filenames), 'self.all_compare_training_filenames', len(self.all_compare_training_filenames),
                  'self.all_different_training_filenames', len(self.all_different_training_filenames))
            # print('self.all_compare_training_filenames', self.all_compare_training_filenames)
            if restore_iter > 0:
                # all_filenames = all_filenames[batch_image_size * (restore_iter):batch_image_size * (restore_iter + 1)]
                # all_compare_filenames = all_compare_filenames[batch_image_size * (restore_iter):batch_image_size * (restore_iter + 1)]
                all_filenames = all_filenames[batch_image_size * (restore_iter + 1):]
                all_compare_filenames = all_compare_filenames[batch_image_size * (restore_iter + 1):]
                all_different_filenames = all_different_filenames[batch_image_size * (restore_iter + 1):]
                # print('make_batch_tensor:all_filenames', all_filenames)
            # print('make_batch_tensor:all_compare_filenames', all_compare_filenames)
        ...
        filename_queue = tf.train.string_input_producer(tf.convert_to_tensor(all_compare_filenames), shuffle=False)
        image_reader = tf.WholeFileReader()
        _, image_file = image_reader.read(filename_queue)
        image = tf.image.decode_gif(image_file)
        # should be T x C x W x H
        ...
        for i in xrange(self.meta_batch_size):
            ...
            # print('append', 'image.shape', image.shape)
            all_compare_images.append(image)
        print('make_compare_batch_tensor', tf.stack(all_compare_images).shape)

        filename_queue = tf.train.string_input_producer(tf.convert_to_tensor(all_different_filenames), shuffle=False)
        image_reader = tf.WholeFileReader()
        _, image_file = image_reader.read(filename_queue)
        image = tf.image.decode_gif(image_file)
        # should be T x C x W x H
        ...
        for i in xrange(self.meta_batch_size):
            ...
            image = tf.reshape(image, [(self.update_batch_size + self.test_batch_size) * self.T, -1])
            # print('append', 'image.shape', image.shape)
            all_different_images.append(image)
        print('make_different_batch_tensor', tf.stack(all_different_images).shape)

        return tf.stack(all_images), tf.stack(all_compare_images), tf.stack(all_different_images)

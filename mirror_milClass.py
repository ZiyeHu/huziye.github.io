    ...
    with graph.as_default():
        ...
        with tf.variable_scope('model', reuse=tf.AUTO_REUSE) as training_scope:
            ...
            def batch_metalearn(inp):
                ...
                print('fast_weights are',fast_weights)
                # Post-update
                if 'Training' in prefix:
                ...
                if FLAGS.compare_learn:
                    ...
                    if 'Training' in prefix:
                        ...
                        compare_learn_local_outputa, compare_learn_final_eept_preda = forward(conv_out_size, inputa,tf.zeros_like(state_inputa),fast_weights, network_config=network_config)
                    else:
                        compare_learn_local_outputa, compare_learn_final_eept_preda = forward(conv_out_size, inputa, tf.zeros_like(state_inputa),fast_weights,is_training=False, network_config=network_config)
                    ...
                    # Compute fast gradients
                    grads = tf.gradients(compare_learn_local_lossa, fast_weights.values())
                    # print('grads',grads)
                    ...
                    if FLAGS.pretrain_weight_path != 'N/A':
                        ...
                        gradients['bc1'] = tf.zeros_like(gradients['bc1'])
                    # gradients_summ.append([gradients[key] for key in sorted_weight_keys])
                    compare_learn_fast_weights = dict(zip(fast_weights.keys(), [fast_weights[key] - step_size * gradients[key] for key in fast_weights.keys()]))
                ...
                # sum_loss = euclidean_loss_layer(0.5 * (outputb - compare_learn_outputb), compare_action, multiplier=loss_multiplier, use_l1=FLAGS.use_l1_l2_loss)
                ...
                local_lossb = multiply_loss = act_loss_eps * left_lossb + act_loss_eps * right_lossb + act_loss_eps * sum_loss
        ...
        train_multiply_op = tf.train.AdamOptimizer(1e-3).minimize(total_multiply_lossb)

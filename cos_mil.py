...
class MIL(object):
    ...
    def init_network(self, graph, input_tensors=None, restore_iter=0, prefix='Training_'):
        ...
        with graph.as_default():
            ...
            outputas, outputbs, test_output, lossesa, lossesb, final_eept_lossesb, flat_img_inputb, gradients, semantic_loss, semantic_outputb, compare_semantic_outputb, different_semantic_outputb = result
            ...
            self.total_semantic_loss = total_semantic_loss = tf.reduce_sum(semantic_loss) / tf.to_float(self.meta_batch_size)
            total_losses2 = [tf.reduce_sum(lossesb[j]) / tf.to_float(self.meta_batch_size) for j in range(self.num_updates)]
            self.total_mix_loss = total_mix_loss = total_semantic_loss + total_losses2[-1]
            ...
            self.semantic_outputb = semantic_outputb
            self.compare_semantic_outputb = compare_semantic_outputb
            self.different_semantic_outputb = different_semantic_outputb
            ...
            if 'Training' in prefix:
                ...
                self.train_op = tf.train.AdamOptimizer(self.meta_lr).minimize(self.total_mix_loss)
                # Add summaries
    ...
    def construct_model(self, input_tensors=None, prefix='Training_', dim_input=27, dim_output=7, network_config=None):
        ...
        inputc = tf.concat(axis=2, values=[tf.zeros_like(statea), obsc])
        ...
        with tf.variable_scope('model', reuse=None) as training_scope:
            ...
            if 'weights' not in dir(self):
                if FLAGS.learn_final_eept:
                    ...
                    #     self.inverse_weights = inverse_weights = self.construct_weights(dim_input, dim_output - len(final_eept_range),network_config=network_config)

                else:
                    self.weights = weights = self.construct_weights(dim_input, dim_output, network_config=network_config)
                    # with tf.variable_scope('inverse'):
                    #     self.inverse_weights = inverse_weights = self.construct_weights(dim_input, dim_output, network_config=network_config)
            ...
            # print('inverse_weights are', inverse_weights)
            ...
            def batch_metalearn(inp):
                ...
                if FLAGS.learn_final_eept:
                    ...
                    # actionc = actionc[:, :final_eept_range[0]]
                ...
                inputc, _, _ = self.construct_image_input(inputc, self.state_idx, self.img_idx, network_config=network_config)
                ...
                # Pre-update
                if 'Training' in prefix:
                    local_outputa, final_eept_preda = self.forward(inputa, tf.zeros_like(state_inputa), weights, network_config=network_config)
                else:
                    local_outputa, final_eept_preda = self.forward(inputa, tf.zeros_like(state_inputa), weights, is_training=False, network_config=network_config)
                if FLAGS.learn_final_eept:
                ...
                if 'Training' in prefix:
                    ...
                    compare_outputb, _ = self.forward(inputa, state_inputa, fast_weights, meta_testing=True, network_config=network_config)
                    different_outputb, _ = self.forward(inputc, tf.zeros_like(state_inputa), fast_weights, meta_testing=True,  network_config=network_config)
                else:
                    ...
                    compare_outputb, _= self.forward(inputa, state_inputa, fast_weights, meta_testing=True, is_training=False, testing=testing, network_config=network_config)
                    different_outputb, _ = self.forward(inputc, tf.zeros_like(state_inputa), fast_weights, meta_testing=True, is_training=False, testing=testing, network_config=network_config)
                ...
                semantic_outputb = norm_output_layer(semantic_outputb)
                ...
                compare_semantic_outputb = norm_output_layer(compare_semantic_outputb)
                ...
                different_semantic_outputb = norm_output_layer(different_semantic_outputb)
                ...
                # semantic_loss = FLAGS.margin_coefficient * act_loss_eps * kl_loss_layer(semantic_outputb, compare_semantic_outputb) + \
                #                 FLAGS.margin_coefficient * act_loss_eps * kl_loss_layer(semantic_outputb, different_semantic_outputb, margin = FLAGS.margin) + \
                #                 FLAGS.margin_coefficient * act_loss_eps * kl_loss_layer(compare_semantic_outputb, different_semantic_outputb, margin = FLAGS.margin)

                # semantic_loss = FLAGS.margin - FLAGS.margin_coefficient * act_loss_eps * kl_loss_layer(semantic_outputb, compare_semantic_outputb) - \
                #                 FLAGS.margin_coefficient * act_loss_eps * kl_loss_layer(semantic_outputb, different_semantic_outputb) + \
                #                 FLAGS.margin_coefficient * act_loss_eps * kl_loss_layer(compare_semantic_outputb, different_semantic_outputb)

                semantic_loss = FLAGS.margin - FLAGS.margin_coefficient * act_loss_eps * cosine_loss_layer(semantic_outputb, compare_semantic_outputb) + \
                                FLAGS.margin_coefficient * act_loss_eps * cosine_loss_layer(semantic_outputb, different_semantic_outputb)

                semantic_loss = tf.maximum(0.0, semantic_loss)


                # local_lossb = act_loss_eps * euclidean_loss_layer(outputb, actionb, multiplier=loss_multiplier, use_l1=FLAGS.use_l1_l2_loss) + \
                #                 0.001 * act_loss_eps * euclidean_loss_layer(compare_outputb, actiona, multiplier=loss_multiplier, use_l1=FLAGS.use_l1_l2_loss)

                local_lossb = act_loss_eps * euclidean_loss_layer(outputb, actionb, multiplier=loss_multiplier, use_l1=FLAGS.use_l1_l2_loss) + \
                               euclidean_loss_layer(compare_outputb, actiona, multiplier=loss_multiplier, use_l1=FLAGS.use_l1_l2_loss)
                ...
                #     print('updating iter: ',j+1)
                #     # Pre-update
                #     state_inputa_new = state_inputas[j+1]
                #     if FLAGS.no_state:
                #         state_inputa_new = None
                #     if 'Training' in prefix:
                #         outputa, final_eept_preda = self.forward(inputas[j+1], tf.zeros_like(state_inputa_new), fast_weights, network_config=network_config)
                #     else:
                #         outputa, final_eept_preda = self.forward(inputas[j+1], tf.zeros_like(state_inputa_new), fast_weights, is_training=False, testing=testing, network_config=network_config)
                #     if FLAGS.learn_final_eept:
                ...
                #     else:
                #         final_eept_lossa = tf.constant(0.0)
                #     loss = act_loss_eps * euclidean_loss_layer(outputa, tf.zeros_like(actionas[j+1]), multiplier=loss_multiplier, use_l1=FLAGS.use_l1_l2_loss)
                #     if FLAGS.learn_final_eept:
                ...
                #         clone_output, _ = self.forward(inputas[j + 1], state_inputa_new, fast_weights, meta_testing=True, network_config=network_config)
                #
                #     else:
                ...
                #         clone_output, _ = self.forward(inputas[j + 1], state_inputa_new, fast_weights, meta_testing=True, is_training=False, testing=testing, network_config=network_config)
                #
                ...
                #     else:
                #         final_eept_lossb = tf.constant(0.0)
                #     lossb = 0.5 * (act_loss_eps * euclidean_loss_layer(output, actionb, multiplier=loss_multiplier, use_l1=FLAGS.use_l1_l2_loss) + \
                #             act_loss_eps * euclidean_loss_layer(clone_output, actiona, multiplier=loss_multiplier, use_l1=FLAGS.use_l1_l2_loss))
                #     if FLAGS.learn_final_eept:
                ...
                local_fn_output = [local_outputa, local_outputbs, local_outputbs[-1], local_lossa, local_lossesb, final_eept_lossesb, flat_img_inputb, gradients_summ, semantic_loss,
                                   semantic_outputb, compare_semantic_outputb, different_semantic_outputb]
                return local_fn_output

        if self.norm_type:
            # initialize batch norm vars.
            unused = batch_metalearn((inputa[0], inputb[0], inputc[0], actiona[0], actionb[0]))

        out_dtype = [tf.float32, [tf.float32]*num_updates, tf.float32, tf.float32, [tf.float32]*num_updates, [tf.float32]*num_updates, tf.float32, [[tf.float32]*len(self.weights.keys())]*num_updates, tf.float32,
                      tf.float32, tf.float32, tf.float32]
        result = tf.map_fn(batch_metalearn, elems=(inputa, inputb, inputc, actiona, actionb), dtype=out_dtype)
        print 'Done with map.'
        return result

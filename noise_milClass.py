    ...
    with graph.as_default():
        ...
        with tf.variable_scope('model', reuse=tf.AUTO_REUSE) as training_scope:
            ...
            def batch_metalearn(inp):
                ...
                # Noise Pre-update
                disturb_weights = dict(zip(fast_weights.keys(), [fast_weights[key] - global_precision * disturb for key in fast_weights.keys()]))
                ...
                fast_weights = dict(zip(fast_weights.keys(), [fast_weights[key] - step_size * disturb_gradients[key] for key in fast_weights.keys()]))

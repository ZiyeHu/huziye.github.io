...
flags.DEFINE_string('experiment', 'sim_push', 'sim_vision_reach or sim_push')
flags.DEFINE_string('demo_file', 'mil_data/data/sim_push_test/',
                    'path to the directory where demo files that containing robot states and actions are stored')
flags.DEFINE_string('demo_gif_dir', 'mil_data/data/sim_push_test/', 'path to the videos of demonstrations')
flags.DEFINE_string('gif_prefix', 'object', 'prefix of the video directory for each task, e.g. object_0 for task 0')
flags.DEFINE_integer('im_width', 125,
                     'width of the images in the demo videos,  125 for sim_push, and 80 for sim_vision_reach')
flags.DEFINE_integer('im_height', 125,
...
flags.DEFINE_integer('meta_batch_size', 20,'number of tasks sampled per meta-update')  # 25 for reaching, 15 for pushing, 12 for placing
...
flags.DEFINE_float('train_update_lr', 0.01,'step size alpha for inner gradient update.')  # 0.001 for reaching, 0.01 for pushing and placing
...
flags.DEFINE_float('loss_multiplier', 50.0,
...
flags.DEFINE_integer('num_strides', 4,
...
# flags.DEFINE_string('log_dirs', 'logs/sim_push_with_bicycle', 'directory for summaries and checkpoints.')
...
flags.DEFINE_integer('action_size', 7, 'size of embedding')
flags.DEFINE_float('margin', 1, 'margin of loss')
flags.DEFINE_float('margin_coefficient', 1, 'margin of loss')
flags.DEFINE_bool('tensor_input', True, 'input by tensor')

XML_PATH = 'sim_push_xmls/'
SCALE_FILE_PATH = 'mil_data/data/scale_and_bias_sim_push.pkl'
CROP = False
...
def load_demo(task_id, demo_dir, demo_inds):
    ...
    # read in demo video
    if CROP:
        demo_gifs = [imageio.mimread(demo_dir + 'crop_object_' + task_id + '/cond%d.samp0.gif' % demo_ind) for demo_ind
                     in demo_inds]
    else:
        demo_gifs = [imageio.mimread(demo_dir + 'object_' + task_id + '/cond%d.samp0.gif' % demo_ind) for demo_ind in
                     demo_inds]
...
def construct_fc_weights(dU, img_idx, state_idx, norm_type='layer_norm', dim_input=27, dim_output=7,
    ...
    dim_output = dim_output + FLAGS.embed_size
    print('dim_output of construct_fc_weights', dim_output)
...
def fc_forward(fc_input, weights, state_input=None, meta_testing=False, is_training=True, testing=False,
...
def main():
    ...
    with graph.as_default():
        ...
        if FLAGS.train and FLAGS.tensor_input:
            data_generator.generate_batches(noisy=FLAGS.use_noisy_demos)

            print('input type is tensor_input:')
            train_image_tensors = data_generator.make_compare_batch_tensor(network_config, restore_iter=FLAGS.restore_iter)

            global_obsa = train_image_tensors[:, :FLAGS.update_batch_size * FLAGS.T, :]
            global_obsb = train_image_tensors[:, FLAGS.update_batch_size * FLAGS.T:(FLAGS.update_batch_size +1) * FLAGS.T, :]
            global_different_obs = train_image_tensors[:, (FLAGS.update_batch_size + 1) * FLAGS.T:, :]
            ...
            global_different_state = tf.placeholder(tf.float32, name='different_state')
            ...
            global_different_input = tf.concat(axis=2, values=[global_different_state, global_different_obs])
            ...
            global_compare_obs = tf.placeholder(tf.float32, name='compare_obs')
            global_different_obs = tf.placeholder(tf.float32, name='different_obs')
        ...
        with tf.variable_scope('model', reuse=tf.AUTO_REUSE) as training_scope:
            ...
            def batch_metalearn(inp):
                ...
                inputa, inputb, compare_input, different_input, actiona, actionb, compare_action = inp
                ...
                different_input = tf.reshape(different_input, [-1, dim_input])
                ...
                print('actiona, actionb', actiona.shape, actionb.shape)
                ...
                #     compare_action = tf.zeros_like(compare_action)global_different_obs = train_image_tensors[:, (FLAGS.update_batch_size +1) * FLAGS.T:, :]
                ...
                compare_input, _, compare_state_input = construct_image_input(compare_input, state_idx, img_idx,
                                                                              network_config=network_config)
                different_input, _, different_state_input = construct_image_input(different_input, state_idx, img_idx,
                                                                                  network_config=network_config)
                inputa, _, state_inputa = construct_image_input(inputa, state_idx, img_idx,
                                                                network_config=network_config)
                inputb, flat_img_inputb, state_inputb = construct_image_input(inputb, state_idx, img_idx,
                                                                              network_config=network_config)

                # print('inputa, inputb',inputa.shape, inputb.shape)
                ...
                if 'Training' in prefix:
                    local_outputa, final_eept_preda = forward(conv_out_size, inputa, tf.zeros_like(state_inputa),
                                                              weights, network_config=network_config)
                else:
                    local_outputa, final_eept_preda = forward(conv_out_size, inputa, tf.zeros_like(state_inputa),
                                                              weights, is_training=False, network_config=network_config)

                print('local_outputa', local_outputa.shape)
                # semantic_outputa = local_outputa[:, :,FLAGS.action_size:]
                # local_outputa = local_outputa[:, :, :FLAGS.action_size]
                # print('semantic_outputa',semantic_outputa.shape,'local_outputa',local_outputa.shape)

                if FLAGS.learn_final_eept:
                    final_eept_lossa = euclidean_loss_layer(final_eept_preda, final_eepta, multiplier=loss_multiplier,
                ...
                fast_weights = dict(
                    zip(weights.keys(), [weights[key] - step_size * gradients[key] for key in weights.keys()]))

                if 'Training' in prefix:
                    compare_outputb, _ = forward(conv_out_size, inputa, state_inputa, fast_weights, meta_testing=True,
                                                 network_config=network_config)
                else:
                    compare_outputb, _ = forward(conv_out_size, inputa, state_inputa, fast_weights, meta_testing=True,
                                                 is_training=False, testing=testing, network_config=network_config)

                # Post-update
                if 'Training' in prefix:
                    outputb, final_eept_predb = forward(conv_out_size, inputb, state_inputb, fast_weights,
                                                        meta_testing=True, network_config=network_config)
                else:
                    outputb, final_eept_predb = forward(conv_out_size, inputb, state_inputb, fast_weights,
                                                        meta_testing=True, is_training=False, testing=testing,
                                                        network_config=network_config)

                # if 'Training' in prefix:
                #     compare_outputb, _  = forward(conv_out_size, compare_input, compare_state_input, fast_weights,meta_testing=True, network_config=network_config)
                # else:
                #     compare_outputb, _  = forward(conv_out_size, compare_input, compare_state_input, fast_weights,meta_testing=True, is_training=False, testing=testing,network_config=network_config)

                if 'Training' in prefix:
                    different_outputb, _ = forward(conv_out_size, different_input, different_state_input, fast_weights,
                                                   meta_testing=True, network_config=network_config)
                else:
                    different_outputb, _ = forward(conv_out_size, different_input, different_state_input, fast_weights,
                                                   meta_testing=True, is_training=False, testing=testing,
                ...
                semantic_outputb = outputb[:, FLAGS.action_size:]
                # semantic_outputb = tf.nn.softmax(semantic_outputb)
                semantic_outputb = tf.nn.sigmoid(semantic_outputb)
                outputb = outputb[:, 0:FLAGS.action_size]

                compare_semantic_outputb = compare_outputb[:, FLAGS.action_size:]
                # compare_semantic_outputb = tf.nn.softmax(compare_semantic_outputb)
                compare_semantic_outputb = tf.nn.sigmoid(compare_semantic_outputb)
                compare_outputb = compare_outputb[:, 0:FLAGS.action_size]

                different_semantic_outputb = different_outputb[:, FLAGS.action_size:]
                # different_semantic_outputb = tf.nn.softmax(different_semantic_outputb)
                different_semantic_outputb = tf.nn.sigmoid(different_semantic_outputb)
                different_outputb = different_outputb[:, 0:FLAGS.action_size]

                print('semantic_output', semantic_outputb.shape, 'outputb', outputb.shape)

                semantic_loss = FLAGS.margin_coefficient * act_loss_eps * kl_loss_layer(semantic_outputb,  compare_semantic_outputb) + \
                                tf.abs((FLAGS.margin - FLAGS.margin_coefficient * act_loss_eps *  kl_loss_layer(semantic_outputb, different_semantic_outputb))) + \
                                tf.abs((FLAGS.margin - FLAGS.margin_coefficient * act_loss_eps * kl_loss_layer(compare_semantic_outputb, different_semantic_outputb)))

                # semantic_loss = FLAGS.margin_coefficient * act_loss_eps * euclidean_loss_layer(semantic_outputb,
                #                                                                                compare_semantic_outputb,
                #                                                                                multiplier=loss_multiplier,
                #                                                                                use_l1=FLAGS.use_l1_l2_loss) + \
                #                 tf.abs((FLAGS.margin - FLAGS.margin_coefficient * act_loss_eps * euclidean_loss_layer(
                #                     semantic_outputb, different_semantic_outputb, multiplier=loss_multiplier,
                #                     use_l1=FLAGS.use_l1_l2_loss))) + \
                #                 tf.abs((FLAGS.margin - FLAGS.margin_coefficient * act_loss_eps * euclidean_loss_layer(
                #                     compare_semantic_outputb, different_semantic_outputb, multiplier=loss_multiplier,
                #                     use_l1=FLAGS.use_l1_l2_loss)))

                print('semantic_loss', semantic_loss.shape)

                local_lossb = 0.5 * (
                            act_loss_eps * euclidean_loss_layer(compare_outputb, actiona, multiplier=loss_multiplier,
                                                                use_l1=FLAGS.use_l1_l2_loss) +
                            act_loss_eps * euclidean_loss_layer(outputb, actionb, multiplier=loss_multiplier,
                                                                use_l1=FLAGS.use_l1_l2_loss))

                # semantic_loss = tf.zeros_like(local_lossb)
                # semantic_loss = act_loss_eps * euclidean_loss_layer(outputb, actionb, multiplier=loss_multiplier,use_l1=FLAGS.use_l1_l2_loss)

                local_outputbs.append(outputb)
                if FLAGS.learn_final_eept:
                    final_eept_lossb = euclidean_loss_layer(final_eept_predb, final_eeptb, multiplier=loss_multiplier,
                                                            use_l1=FLAGS.use_l1_l2_loss)
                else:
                ...
                local_fn_output = [local_outputa, local_outputbs, local_outputbs[-1], local_lossa, local_lossesb,
                                   final_eept_lossesb, flat_img_inputb, semantic_loss, semantic_outputb, compare_semantic_outputb, different_semantic_outputb]
        ...
        if norm_type and FLAGS.train:
            # initialize batch norm vars.
            print('initialize batch norm vars****************************************************************')
            unused = batch_metalearn((global_inputa[0], global_inputb[0], global_compare_input[0],
                                      global_different_input[0], global_actiona[0],
                                      global_actionb[0], global_compare_action[0]))
        ...
        out_dtype = [tf.float32, [tf.float32] * FLAGS.num_updates, tf.float32, tf.float32,
                     [tf.float32] * FLAGS.num_updates,
                     [tf.float32] * FLAGS.num_updates, tf.float32, tf.float32, tf.float32, tf.float32, tf.float32]
        # result = tf.map_fn(batch_metalearn, elems=(global_inputa, global_inputb,global_compare_input, global_different_input, global_actiona, global_actionb,global_compare_action), dtype=out_dtype)
        ...
        if FLAGS.train:
            with tf.device("/cpu:0"):
                ...
                tower_grads = []
                batch_size = int(FLAGS.meta_batch_size / num_gpus)
                opt = tf.train.AdamOptimizer(FLAGS.meta_lr)
                print('num_gpus', num_gpus, 'batch_size', batch_size)
                with tf.variable_scope(tf.get_variable_scope()):
                    for i in range(num_gpus):
                        with tf.device('/gpu:%d' % i):
                            with tf.name_scope('gpu_%d' % i) as scope:
                                with tf.name_scope("tower_%d" % i):
                                    part_inputa = global_inputa[i * batch_size:(i + 1) * batch_size]
                                    part_inputb = global_inputb[i * batch_size:(i + 1) * batch_size]
                                    part_compare_input = global_compare_input[i * batch_size:(i + 1) * batch_size]
                                    part_different_input = global_different_input[i * batch_size:(i + 1) * batch_size]
                                    part_actiona = global_actiona[i * batch_size:(i + 1) * batch_size]
                                    part_actionb = global_actionb[i * batch_size:(i + 1) * batch_size]
                                    part_compare_action = global_compare_action[i * batch_size:(i + 1) * batch_size]

                                    print('part_inputa', part_inputa.shape, 'part_actiona', part_actiona.shape)
                                    result = tf.map_fn(batch_metalearn, elems=( part_inputa, part_inputb, part_compare_input, part_different_input,
                                        part_actiona, part_actionb, part_compare_action), dtype=out_dtype)
                                    outputas, outputbs, test_output, lossesa, lossesb, final_eept_lossesb, flat_img_inputb, semantic_loss, semantic_outputb, compare_semantic_outputb, different_semantic_outputb = result

                                    print('outputas, outputbs, FLAGS.num_updates',outputas, outputbs, FLAGS.num_updates)


                                    total_semantic_loss = tf.reduce_sum(semantic_loss) / tf.to_float(batch_size)
                                    # total_losses2 = [tf.reduce_sum(lossesb[j]) / tf.to_float(batch_size) for j in range(FLAGS.num_updates)] + total_semantic_loss
                                    total_losses2 = [tf.reduce_sum(lossesb[j]) / tf.to_float(batch_size) for j in range(FLAGS.num_updates)]
                                    cur_grads = opt.compute_gradients(total_losses2[FLAGS.num_updates - 1])
            ...
            global_inputa, global_inputb, global_compare_input, global_different_input, global_actiona,
            global_actionb, global_compare_action), dtype=out_dtype)
            ...
            total_semantic_loss = tf.reduce_sum(semantic_loss) / tf.to_float(FLAGS.meta_batch_size)
            # total_losses2 = [tf.reduce_sum(lossesb[j]) / tf.to_float(FLAGS.meta_batch_size) for j in   range(FLAGS.num_updates)] + total_semantic_loss
            ...
            # global_semantic_outputb = semantic_outputb
            # global_compare_semantic_outputb = compare_semantic_outputb
            # global_different_semantic_outputb = different_semantic_outputb
        ...
        def train(graph, saver, sess, data_generator, log_dirs, restore_itr=0):
            ...
            for itr in training_range:
                ...
                # train_image_datas, train_compare_image_datas, train_different_image_datas = data_generator.make_compare_batch_tensor(network_config, restore_iter=itr)
                # train_image_datas, train_compare_image_datas, train_different_image_datas = data_generator.make_compare_batch_data(network_config, restore_iter=itr)

                # print('train_image_datas', train_image_datas.shape, 'train_compare_image_datas', train_compare_image_datas.shape, 'train_different_image_datas', train_different_image_datas.shape)
                ...
                # different_obsa_input = train_different_image_datas[:, :FLAGS.update_batch_size * FLAGS.T, :]
                # different_obsb_input = train_different_image_datas[:, FLAGS.update_batch_size * FLAGS.T:, :]
                ...
                feed_dict = {
                    ...
                    global_different_state: np.zeros_like(compare_statea_input),
                ...
                #              global_different_obs: different_obsa_input,
                #
                #              global_statea: statea_input,
                #              global_stateb: stateb_input,
                #              global_compare_state: compare_statea_input,
                ...
                # input_tensors = [train_op, total_losses2, total_semantic_loss]

                input_tensors = [train_op, global_semantic_outputb, global_compare_semantic_outputb, global_different_semantic_outputb, total_losses2, total_semantic_loss]


                results = sess.run(input_tensors, feed_dict=feed_dict)
                print '(Phase 1)Iteration %d:  average loss is %.2f,  semantic loss is %.2f,' % \
                      (itr, np.mean(results[-2]), np.mean(results[-1]))
                # print(results[1][0][0], results[2][0][0], results[3][0][0])

                with open('logs/sim_reach_temporal_conv/traing_loss.txt', 'a') as f:
                    f.write("%d %f %f\n" % (itr, np.mean(results[-2]), np.mean(results[-1])))

                # with open('logs/sim_push/traing_loss.txt', 'a') as f:
                ...
                # print '(Phase 2)Iteration %d:  average loss is %.2f' % \
        ...
        class TFAgent(object):
            def __init__(self, scale_bias_file, sess):
                self.sess = sess
            ...
            def get_action(self, obs):
                ...
                action = self.sess.run(test_act_op,
                                       {global_statea: self.demoX.dot(self.scale) + self.bias,
                                        global_actiona: self.demoU,
                                        global_compare_state: obs.dot(self.scale) + self.bias})
                return action, dict()
            ...
            def get_vision_action(self, image, obs, t=-1):
                ...
                if self.scale is not None:
                    # print('self.scale is not None')

                    action, semantic_outputb_value, compare_semantic_outputb_value  = self.sess.run([test_act_op, global_semantic_outputb, global_compare_semantic_outputb],
                                           {global_statea: self.demoX.dot(self.scale) + self.bias,
                                            global_obsa: self.demoVideo,
                                            global_actiona: self.demoU,

                                            global_stateb: obs.dot(self.scale) + self.bias,
                                            global_obsb: image,

                                            global_compare_state: obs.dot(self.scale) + self.bias,
                                            global_compare_obs: image})

                    # print('semantic_outputb_valu', semantic_outputb_value, 'compare_semantic_outputb_value',
                    #       compare_semantic_outputb_value[0][-1])
                    # print('semantic_outputb_valu', semantic_outputb_value, 'compare_semantic_outputb_value', compare_semantic_outputb_value)
                    # print('self.scale is not None','self.demoX.shape',self.demoX.shape,'self.demoU.shape',self.demoU.shape)
                    #
                else:
                    print('self.scale is None')
        ...
        def evaluate_push(sess, graph, exp_string, log_dir, demo_dir, id=-1, save_video=False,
                          num_input_demos=1):
            ...
            for task_id in task_ids:
                ...
                policy = TFAgent(scale_file, sess)
                ...
                while True:
                    ...
                    path = rollout(env, policy, max_path_length=100, env_reset=True,
                                   animated=True, speedup=1, always_return_paths=True, save_video=save_video,
                                   video_filename=video_suffix, vision=True)
                    num_trials += 1
            ...
            with open('logs/log_sim_reach_temporal_conv.txt', 'a') as f:
                f.write(exp_string + ':\n')
        ...
        def evaluate_vision_reach(env, graph, data_generator, sess, exp_string, record_gifs, log_dirs):
            ...
            with open('logs/sim_reach_temporal_conv/log_sim_reach_temporal_conv.txt', 'a') as f:
                f.write(exp_string + ':\n')
                f.write(success_rate_msg + '\n')
            ...
            if FLAGS.begin_restore_iter != FLAGS.end_restore_iter:
                ...
                while iter_index <= FLAGS.end_restore_iter:
                    ...
                    if 'reach' in FLAGS.experiment:
                        ...
                        evaluate_push(sess, graph, exp_string, log_dirs, FLAGS.demo_file + '/',

                ...
                with tf.variable_scope(tf.get_variable_scope()):
                    for i in range(num_gpus):
                        with tf.device(assign_to_device('/gpu:{}'.format(i), ps_device='/cpu:0')):
                            ...
                            part_inputa, part_inputb, part_compare_input, part_different_input, part_actiona,
                            part_actionb, part_compare_action), dtype=out_dtype)
                            ...
                            total_losses2 = [tf.reduce_sum(lossesb[j]) / tf.to_float(batch_size) for j in
                                             range(FLAGS.num_updates)] + total_semantic_loss
                            grads = opt.compute_gradients(total_losses2[FLAGS.num_updates - 1])

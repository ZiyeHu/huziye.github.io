...
def main():
    ...
    with graph.as_default():
        ...
        if FLAGS.train and FLAGS.tensor_input:
            ...
            train_image_tensors, train_compare_image_tensors, train_different_image_tensors = data_generator.make_compare_batch_tensor(network_config, restore_iter=FLAGS.restore_iter)

            global_obsa = train_image_tensors[:, :FLAGS.update_batch_size * FLAGS.T, :]
            global_obsb = train_image_tensors[:, :FLAGS.update_batch_size * FLAGS.T, :]
            global_compare_obs = train_compare_image_tensors[:, :FLAGS.update_batch_size * FLAGS.T, :]
            global_different_obs = train_different_image_tensors[:, :FLAGS.update_batch_size * FLAGS.T, :]
        ...
        with tf.variable_scope('model', reuse=tf.AUTO_REUSE) as training_scope:
            ...
            def batch_metalearn(inp):
                ...
                different_input, _, different_state_input = construct_image_input(different_input, state_idx, img_idx,network_config=network_config)
                ...
                if 'Training' in prefix:
                    compare_outputb, _  = forward(conv_out_size, inputa, state_inputa, fast_weights,meta_testing=True, network_config=network_config)
                else:
                    compare_outputb, _  = forward(conv_out_size, inputa, state_inputa, fast_weights,meta_testing=True, is_training=False, testing=testing,network_config=network_config)
                ...
                if 'Training' in prefix:
                    different_outputb, _  = forward(conv_out_size, different_input, different_state_input, fast_weights,meta_testing=True, network_config=network_config)
                else:
                    different_outputb, _  = forward(conv_out_size, different_input, different_state_input, fast_weights,meta_testing=True, is_training=False, testing=testing,network_config=network_config)
                ...
                semantic_loss =  FLAGS.margin_coefficient * act_loss_eps * euclidean_loss_layer(semantic_outputb, compare_semantic_outputb, multiplier=loss_multiplier,use_l1=FLAGS.use_l1_l2_loss) +\
                                 tf.abs((FLAGS.margin - FLAGS.margin_coefficient *  act_loss_eps * euclidean_loss_layer(semantic_outputb, different_semantic_outputb, multiplier=loss_multiplier, use_l1=FLAGS.use_l1_l2_loss)))+ \
                                 tf.abs((FLAGS.margin - FLAGS.margin_coefficient *  act_loss_eps * euclidean_loss_layer(compare_semantic_outputb, different_semantic_outputb, multiplier=loss_multiplier, use_l1=FLAGS.use_l1_l2_loss)))
                ...
                local_lossb = 0.5 * (act_loss_eps * euclidean_loss_layer(compare_outputb, actiona, multiplier=loss_multiplier,use_l1=FLAGS.use_l1_l2_loss) +
                                act_loss_eps * euclidean_loss_layer(outputb, actionb, multiplier=loss_multiplier,use_l1=FLAGS.use_l1_l2_loss))
        ...
        if norm_type:
            ...
            unused = batch_metalearn((global_inputa[0], global_inputb[0], global_compare_input[0], global_different_input[0], global_actiona[0],
        ...
        if FLAGS.train:
                ...
                with tf.variable_scope(tf.get_variable_scope()):
                    for i in range(num_gpus):
                        with tf.device(assign_to_device('/gpu:{}'.format(i), ps_device='/cpu:0')):
                            ...
                            result = tf.map_fn(batch_metalearn, elems=(part_inputa, part_inputb, part_compare_input, part_different_input, part_actiona, part_actionb, part_compare_action), dtype=out_dtype)
            ...
            result = tf.map_fn(batch_metalearn, elems=(global_inputa, global_inputb, global_compare_input, global_different_input, global_actiona,
        ...
        def evaluate_vision_reach(env, graph, data_generator, sess, exp_string, record_gifs, log_dirs):
            ...
            with open('logs/sim_push/log_sim_vision_reach.txt', 'a') as f:
                f.write(exp_string + ':\n')
                f.write(success_rate_msg + '\n')
            ...
            if FLAGS.begin_restore_iter!=FLAGS.end_restore_iter:
                ...
                while iter_index<=FLAGS.end_restore_iter:
                    ...
                    if 'reach' in FLAGS.experiment:
                        ...
                        evaluate_push(sess, graph, exp_string, log_dirs, FLAGS.demo_file + '/',save_video=FLAGS.record_gifs)
                    iter_index+=100
            else:
                if FLAGS.restore_iter > 0:
                    model_file = model_file[:model_file.index('model')] + 'model_' + str(FLAGS.restore_iter)

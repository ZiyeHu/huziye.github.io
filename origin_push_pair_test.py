...
def train(graph, model, saver, sess, data_generator, log_dir, restore_itr=0):
    ...
    for itr in training_range:
        ...
        input_tensors = [model.train_op, model.total_loss1, model.total_losses2[model.num_updates-1]]
        ...
        with graph.as_default():
            ...
            with open('logs/sim_push/push_traing_loss.txt', 'a') as f:
                f.write("%d %f \n" % (itr, np.mean(results[-1])))

        print 'Iteration %d: average loss is %.2f' % (itr, results[-1])

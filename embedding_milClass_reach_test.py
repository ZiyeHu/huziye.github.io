...
flags.DEFINE_string('demo_file', 'mil_data/data/sim_vision_reach_test/',
...
flags.DEFINE_integer('embed_temporal_filter_size', 1, 'filter size for embedding temporal convolution')
...
flags.DEFINE_integer('begin_restore_iter', 100, 'iteration to load model (-1 for latest model)')
...
flags.DEFINE_integer('embed_size', 2, 'size of embedding')
    ...
    for i in xrange(n_layers):
        ...
        if (i == n_layers - 1 or (i == 0 and FLAGS.zero_state and not FLAGS.two_arms)) and FLAGS.two_head:
            ...
            if i == n_layers - 1 and FLAGS.temporal_conv_2_head:
                ...
                temporal_kernel_size = FLAGS.embed_temporal_filter_size
        ...
        if not pretrain or i != 0:
            ...
            if norm_type == 'selu':
                ...
                if i != 0:
                    ...
                    weights['wc%d' % (i + 1)] = init_conv_weights_xavier(
                        [filter_sizes[i], filter_sizes[i], fan_in + FLAGS.embed_size, num_filters[i]],
                        name='wc%d' % (i + 1))
                ...
                if i != 0:
                    ...
                    weights['wc%d' % (i + 1)] = init_weights([filter_sizes[i], filter_sizes[i], fan_in + FLAGS.embed_size, num_filters[i]],
                                                             name='wc%d' % (i + 1))
            ...
            print(i,'construct weights', weights['wc%d' % (i + 1)] ,weights['embed_wc%d' % (i + 1)])
...
def fc_forward(fc_input, weights, state_input=None, meta_testing=False, is_training=True, testing=False,
    ...
    for i in xrange(n_layers):
        ...
        if (i == n_layers - 1 or (
            ...
            print(i, 'forward fc_weights', fc_output, weights['w_%d' % i])
            # if i == 1:
...
def forward(conv_out_size, image_input, state_input, weights, meta_testing=False, is_training=True,
            testing=False,
            norm_type='layer_norm', network_config=None, activation_fn=tf.nn.relu):
    """ Perform the forward pass. """
    ...
    if FLAGS.fc_bt:
        ...
        im_width = network_config['image_width']
        num_channels = network_config['image_channels']
        flatten_image = tf.reshape(image_input, [-1, im_height * im_width * (num_channels + FLAGS.embed_size)])
        context = tf.transpose(tf.gather(tf.transpose(tf.zeros_like(flatten_image)), range(FLAGS.bt_dim)))
        context += weights['context']
        if FLAGS.learn_final_eept:
            context_final_eept = tf.transpose(
    ...
    with graph.as_default():
        ...
        with tf.variable_scope('model', reuse=tf.AUTO_REUSE) as training_scope:
            ...
            def batch_metalearn(inp):
                ...
                print('embed_outputa is',embed_outputa.shape)
                embed_outputa = tf.reshape(embed_outputa, [-1, FLAGS.embed_size])
                embed_outputa = tf.tile(embed_outputa, [FLAGS.T, 1])
                embed_outputa = tf.reshape(embed_outputa,[-1,1,1,embed_outputa.shape[-1]])
                embed_outputa = tf.tile(embed_outputa, [1, inputa.shape[1], inputa.shape[2], 1])
                print('pre embed_outputa.shape', embed_outputa.shape)

                inputa=tf.concat([inputa,embed_outputa],axis=-1)
                print('pre concat inputa',inputa.shape)
                # Pre-update
                if 'Training' in prefix:
                    local_outputa, final_eept_preda = forward(conv_out_size, inputa, tf.zeros_like(state_inputa), weights,network_config=network_config)
                else:
                    local_outputa, final_eept_preda = forward(conv_out_size, inputa,  tf.zeros_like(state_inputa), weights, is_training=False,network_config=network_config)
                ...
                embed_outputa = tf.reshape(embed_outputa, [-1, 1, 1, embed_outputa.shape[-1]])
                embed_outputa = tf.tile(embed_outputa, [1, inputb.shape[1], inputb.shape[2], 1])
                print('post embed_outputa.shape', embed_outputa.shape)

                inputb = tf.concat([inputb, embed_outputa], axis=-1)
                print('post concat inputb', inputb.shape)

                # Post-update
                if 'Training' in prefix:
                    outputb, final_eept_predb = forward(conv_out_size, inputb, state_inputb, fast_weights, meta_testing=True, network_config=network_config)
                else:
                    outputb, final_eept_predb = forward(conv_out_size, inputb, state_inputb, fast_weights, meta_testing=True, is_training=False,testing=testing, network_config=network_config)
                ...
                right_lossb = left_lossb = act_loss_eps * euclidean_loss_layer(outputb, tf.zeros_like(outputb),
        ...
        outputas, outputbs, test_output, lossesa, lossesb, left_lossb, right_lossb = result
        ...
        def train(graph, saver, sess, data_generator, log_dirs, restore_itr=0):
            ...
            for itr in training_range:
                ...
                train_image_datas, train_compare_image_datas, train_goal_image_datas = data_generator.make_compare_batch_data(
                    network_config, restore_iter=itr)
                ...
                feed_dict = {global_obsa: obsa_input,
                             ...
                             global_compare_obsb: obsb_goal_input,
        ...
        def evaluate_vision_reach(env, graph, data_generator, sess, exp_string, record_gifs, log_dirs):
            ...
            for i in xrange(len(selected_demo['selected_demoX'])):
                ...
                print('compare_selected_demoO.shape',compare_selected_demoO.shape)
                ...
                for j in xrange(REACH_DEMO_CONDITIONS):
                    if j in data_generator.demos[i]['demoConditions']:
                        ...
                        for t in range(T):
                            ...
                            # print('selected_demoO, obs, compare_selected_demoO', selected_demoO.shape, obs.shape, compare_selected_demoO.shape)
                            feed_dict = {

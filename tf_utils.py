...
def euclidean_loss_layer(a, b, multiplier=100.0, use_l1=False, eps=0.01):
    ...
    uP = tf.reduce_mean(tf.square(uP))

    print('euclidean_loss_layer', uP.shape)

    return uP

def kl_loss_layer(a, b, multiplier=1.0, use_l1=False, eps=0.01, margin=0.0):

    # multiplier = tf.constant(multiplier, dtype='float')  # for bc #10000
    uP = a * tf.log(a/b)
    uP = tf.reduce_sum(uP, axis=-1)
    uP = tf.abs(margin - uP)
    uP = tf.reduce_mean(uP)

    print('kl_loss_layer', uP.shape)
...
# #cos_average
# def cosine_loss_layer(a, b, multiplier=1.0, use_l1=False, eps=0.01, margin=0.0):
#     uP = tf.multiply(a, b)
#     uP = tf.reduce_sum(uP, axis=-1)
#     uP = tf.reduce_mean(uP)
#     print('cosine_loss_layer', uP.shape)
#     return uP

#cos_last
def cosine_loss_layer(a, b, multiplier=1.0, use_l1=False, eps=0.01, margin=0.0):

    uP = tf.multiply(a[-1], b[-1])
    uP = tf.reduce_sum(uP, axis=-1)
    uP = tf.reduce_mean(uP)
    print('cosine_loss_layer', uP.shape)
    return uP
...
#
#     multiplier = tf.constant(multiplier, dtype='float')  # for bc #10000
#     a = tf.clip_by_value(a, 1e-8, 1.0)
#     b = tf.clip_by_value(b, 1e-8, 1.0)
#     uP = a * tf.log(a/b)
#     uP = tf.reduce_sum(uP, axis=-1)
#     uP = tf.abs(margin - uP)
#     uP = tf.reduce_mean(uP)
...
#     b = tf.reduce_mean(b, axis=0) + 1e-10
#     uP = a * tf.log(a / b)
#     print('uP', uP.shape)
#
#     uP = tf.reduce_sum(uP, axis=-1)
#     print('tf.reduce_sum uP', uP.shape)
#
#     uP = tf.abs(margin - uP)
#     uP = tf.reduce_mean(uP)
#
...
def norm_output_layer(a):
    p = tf.pow(a, 2)
    x = tf.reduce_sum(p, axis=-1)
    y = tf.sqrt(x)
    temp = tf.transpose(a, [1, 0])
    z = temp / y
            ...
            if is_training:
                try:
                    layer = tf.contrib.layers.batch_norm(layer, is_training=True, center=True,
                                                         scale=False, decay=decay, activation_fn=activation_fn,
                                                         updates_collections=None, scope=vs)  # updates_collections=None
                except ValueError:
                    layer = tf.contrib.layers.batch_norm(layer, is_training=True, center=True,
                                                         scale=False, decay=decay, activation_fn=activation_fn,
                                                         updates_collections=None, scope=vs,
                                                         reuse=True)  # updates_collections=None
            else:

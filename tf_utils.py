...
def init_weights(shape, name=None):
    ...
    return tf.get_variable(name=name,shape=shape,initializer=tf.constant_initializer(weights), dtype=tf.float32)
    # return safe_get(name, list(shape), initializer=tf.constant_initializer(weights), dtype=tf.float32)

def init_bias(shape, name=None):
    print('init_bias',name)
    return tf.get_variable(name=name,  initializer=tf.zeros(shape, dtype=tf.float32), dtype=tf.float32)
...
def init_fc_weights_xavier(shape, name=None):
    ...
    return tf.get_variable(name=name, shape=shape, initializer=fc_initializer, dtype=tf.float32)
...
def init_conv_weights_xavier(shape, name=None):
    ...
    return tf.get_variable(name=name, shape=shape, initializer=conv_initializer, dtype=tf.float32)
...
def norm(layer, norm_type='batch_norm', decay=0.9, id=0, is_training=True, activation_fn=tf.nn.relu, prefix='conv_'):
    # print('norm_type is',norm_type,'prefix',prefix)
        ...
        if norm_type == 'batch_norm':
            if is_training:
                try:
                    ...
                    layer = tf.contrib.layers.batch_norm(layer, is_training=True, center=True,
                        scale=False, decay=decay, activation_fn=activation_fn, updates_collections=None, scope=vs, reuse=tf.AUTO_REUSE) # updates_collections=None
            else:
            ...
            try:
                # layer = activation_fn(tf.contrib.layers.layer_norm(layer, center=True,trainable=is_training,
                #     scale=False, scope=vs)) # updates_collections=None
                layer = activation_fn(tf.contrib.layers.layer_norm(layer, center=True, scale=False, scope=vs, reuse=tf.AUTO_REUSE))  # updates_collections=None
                
            except ValueError:
                # layer = activation_fn(tf.contrib.layers.layer_norm(layer, center=True,trainable=is_training,
                #     scale=False, scope=vs, reuse=True))
                layer = activation_fn(tf.contrib.layers.layer_norm(layer, center=True,  scale=False, scope=vs,  reuse=tf.AUTO_REUSE))
        elif norm_type == 'selu':
            layer = selu(layer)
        else:

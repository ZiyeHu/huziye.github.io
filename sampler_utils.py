...
def rollout(env, agent, max_path_length=np.inf, animated=False, speedup=1,
    ...
    if vision:
        ...
        # print('image_obs',np.sum(image_obs),'nonimage_obs',np.sum(nonimage_obs))
        sum_obs = 0.0
        sum_obs += np.sum(image_obs)

        # nonimage_obs=0.0*nonimage_obs
        while sum_obs == 0:
            env.render()
            if 'get_current_image_obs' in dir(env):
                image_obs, nonimage_obs = env.get_current_image_obs()
    ...
    while path_length < max_path_length:
        ...
        # print(path_length,'next_o', np.sum(next_o),next_o.shape)
        if vision:
            if 'get_current_image_obs' in dir(env):
            ...
            # next_nonimage_obs = 0.0 * next_nonimage_obs
            # print(path_length,'next_image_obs', np.sum(next_image_obs), 'next_nonimage_obs', np.sum(next_nonimage_obs))
            sum_obs = 0.0
            sum_obs += np.sum(next_image_obs)
            while sum_obs == 0:
                env.render()
                if 'get_current_image_obs' in dir(env):
                    next_image_obs, next_nonimage_obs = env.get_current_image_obs()

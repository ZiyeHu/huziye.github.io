...
## Training options
flags.DEFINE_integer('metatrain_iterations', 30000,
                     'number of metatraining iterations.')  # 30k for pushing, 50k for reaching and placing
flags.DEFINE_integer('meta_batch_size', 25,
                     'number of tasks sampled per meta-update')  # 25 for reaching, 15 for pushing, 12 for placing
...
flags.DEFINE_integer('embed_size', 20, 'size of embedding')
flags.DEFINE_float('margin', 5.0, 'margin of loss')
...
def generate_test_demos(data_generator):
    ...
    for i in xrange(n_folders):
        ...
        for idx in selected_cond:
            ...
            g = np.concatenate([O[0], O[-1]], axis=0)
            # print('goal.shape', g.shape)

            # print('g.shape',g.shape)
            g = g.reshape(1, -1) / 255.0  # normalize
        ...
        compare_Xs.append(
            demos[i]['demoX'][np.arange(demos[i]['demoX'].shape[0]) == compare_policy_demo_idx[i]].squeeze())
        compare_Us.append(
            demos[i]['demoU'][np.arange(demos[i]['demoU'].shape[0]) == compare_policy_demo_idx[i]].squeeze())
    ...
    compare_selected_demo = dict(selected_demoX=compare_selected_demoX, selected_demoU=compare_selected_demoU,
                                 selected_demoO=compare_selected_demoO)
    data_generator.compare_selected_demo = compare_selected_demo



def construct_fc_weights(dU, img_idx, state_idx, norm_type='layer_norm', dim_input=27, dim_output=7,
                         network_config=None):
    ...
    weights = {}
    in_shape = dim_input
    print('construct_fc_weights', n_layers)
    for i in xrange(n_layers):
        if FLAGS.two_arms and i == 0:
            print('construct_fc_weights FLAGS.two_arms and', i)
            if norm_type == 'selu':
                ...
                weights['embed_w_%d_img' % i] = init_fc_weights_snn([in_shape - len(state_idx), dim_hidden[i]],
                                                                    name='embed_w_%d_img' % i)
                weights['embed_w_%d_state' % i] = init_fc_weights_snn([len(state_idx), dim_hidden[i]],
                                                                      name='embed_w_%d_state' % i)
            else:
                ...
                weights['embed_w_%d_img' % i] = init_weights([in_shape - len(state_idx), dim_hidden[i]],
                                                             name='embed_w_%d_img' % i)
                weights['embed_w_%d_state' % i] = init_weights([len(state_idx), dim_hidden[i]],
                                                               name='embed_w_%d_state' % i)

                weights['b_%d_state_two_arms' % i] = init_bias([dim_hidden[i]], name='b_%d_state_two_arms' % i)
                weights['embed_b_%d_state_two_arms' % i] = init_bias([dim_hidden[i]],
                                                                     name='embed_b_%d_state_two_arms' % i)
            weights['b_%d_img' % i] = init_bias([dim_hidden[i]], name='b_%d_img' % i)
            weights['b_%d_state' % i] = init_bias([dim_hidden[i]], name='b_%d_state' % i)

            weights['embed_b_%d_img' % i] = init_bias([dim_hidden[i]], name='embed_b_%d_img' % i)
            weights['embed_b_%d_state' % i] = init_bias([dim_hidden[i]], name='embed_b_%d_state' % i)
        ...
        if norm_type == 'selu':
            ...
            if i != 1:
                ...
                weights['w_%d' % i] = init_weights([in_shape + FLAGS.embed_size, dim_hidden[i]], name='w_%d' % i)
            if i < (n_layers - 1):
                if i != 0:
                    weights['embed_w_%d' % i] = init_weights([FLAGS.layer_size, FLAGS.layer_size],
                                                             name='embed_w_%d' % i)
                else:
                    if FLAGS.fp:
                        weights['embed_w_%d' % i] = init_weights([FLAGS.num_filters * 2, FLAGS.layer_size],
                                                                 name='embed_w_%d' % i)
                    else:
                        weights['embed_w_%d' % i] = init_weights([FLAGS.num_filters, FLAGS.layer_size],
                                                                 name='embed_w_%d' % i)
            else:
                weights['embed_w_%d' % i] = init_weights([FLAGS.layer_size, FLAGS.embed_size], name='embed_w_%d' % i)
            print(i, 'test construct_fc_weights', weights['embed_w_%d' % i], weights['w_%d' % i])

        weights['b_%d' % i] = init_bias([dim_hidden[i]], name='b_%d' % i)

        if i < (n_layers - 1):
            weights['embed_b_%d' % i] = init_bias([FLAGS.layer_size], name='embed_b_%d' % i)
        else:
            weights['embed_b_%d' % i] = init_bias([FLAGS.embed_size], name='embed_b_%d' % i)

        print(i, 'construct_fc_weights', weights['embed_w_%d' % i])
        ...
        if (i == n_layers - 1 or (i == 0 and FLAGS.zero_state and not FLAGS.two_arms)) and FLAGS.two_head:
            ...
            if i == n_layers - 1 and FLAGS.temporal_conv_2_head:
                ...
                for j in xrange(len(temporal_num_filters)):
                    if j != len(temporal_num_filters) - 1:
                        weights['w_1d_conv_2_head_%d' % j] = init_weights(
                            [temporal_kernel_size, in_shape, temporal_num_filters[j]], name='w_1d_conv_2_head_%d' % j)
                        weights['b_1d_conv_2_head_%d' % j] = init_bias([temporal_num_filters[j]],
                                                                       name='b_1d_conv_2_head_%d' % j)

                        weights['embed_w_1d_conv_2_head_%d' % j] = init_weights(
                            [temporal_kernel_size, in_shape, temporal_num_filters[j]],
                            name='embed_w_1d_conv_2_head_%d' % j)
                        weights['embed_b_1d_conv_2_head_%d' % j] = init_bias([temporal_num_filters[j]],
                                                                             name='embed_b_1d_conv_2_head_%d' % j)
                        ...
                        # weights['embed_w_1d_conv_2_head_%d' % j] = init_weights([1, in_shape, temporal_num_filters[j]],name='embed_w_1d_conv_2_head_%d' % j)
                        # weights['embed_b_1d_conv_2_head_%d' % j] = init_bias([temporal_num_filters[j]], name='embed_b_1d_conv_2_head_%d' % j)

                        weights['embed_w_1d_conv_2_head_%d' % j] = init_weights([1, in_shape, FLAGS.embed_size],
                                                                                name='embed_w_1d_conv_2_head_%d' % j)
                        weights['embed_b_1d_conv_2_head_%d' % j] = init_bias([FLAGS.embed_size],
                                                                             name='embed_b_1d_conv_2_head_%d' % j)

                    print('embed j', j, 'temporal', weights['w_1d_conv_2_head_%d' % j],
                          weights['embed_w_1d_conv_2_head_%d' % j])
                ...
                if i == 0:
                    if FLAGS.fp:
                        weights['embed_w_%d_two_heads' % i] = init_weights([FLAGS.num_filters * 2, dim_hidden[i]],
                                                                           name='embed_w_%d_two_heads' % i)
                    else:
                        weights['embed_w_%d_two_heads' % i] = init_weights([FLAGS.num_filters, dim_hidden[i]],
                                                                           name='embed_w_%d_two_heads' % i)
                    weights['embed_b_%d_two_heads' % i] = init_bias([dim_hidden[i]], name='embed_b_%d_two_heads' % i)
                elif i < n_layers - 1:
                    weights['embed_w_%d_two_heads' % i] = init_weights([in_shape, dim_hidden[i]],
                                                                       name='embed_w_%d_two_heads' % i)
                    weights['embed_b_%d_two_heads' % i] = init_bias([dim_hidden[i]], name='embed_b_%d_two_heads' % i)

                print(i, 'construct two_heads', weights['w_%d_two_heads' % i], weights['embed_w_%d_two_heads' % i])
        in_shape = dim_hidden[i]
...
def construct_weights(dU, img_idx, state_idx, norm_type, network_config=None):
    ...
    for i in xrange(n_conv_layers):
        if not pretrain or i != 0:
            # print('norm_type',norm_type,i,num_filters[i])
            if norm_type == 'selu':
                ...
                if i != 0:
                    weights['embed_wc%d' % (i + 1)] = init_conv_weights_snn(
                        [filter_sizes[i], filter_sizes[i], fan_in, num_filters[i]], name='embed_wc%d' % (i + 1))
                else:
                    weights['embed_wc%d' % (i + 1)] = init_conv_weights_snn(
                        [filter_sizes[i], filter_sizes[i], 6, num_filters[i]], name='embed_wc%d' % (i + 1))
                ...
                weights['wc%d' % (i + 1)] = init_conv_weights_xavier(
                    ...
                    name='wc%d' % (i + 1))  # 5x5 conv, 1 input, 32 outputs
                if i != 0:
                    weights['embed_wc%d' % (i + 1)] = init_conv_weights_xavier(
                        [filter_sizes[i], filter_sizes[i], fan_in, num_filters[i]], name='embed_wc%d' % (i + 1))
                else:
                    weights['embed_wc%d' % (i + 1)] = init_conv_weights_xavier(
                        [filter_sizes[i], filter_sizes[i], 6, num_filters[i]], name='embed_wc%d' % (i + 1))
                ...
                weights['wc%d' % (i + 1)] = init_weights([filter_sizes[i], filter_sizes[i], fan_in, num_filters[i]],
                                                         name='wc%d' % (i + 1))  # 5x5 conv, 1 input, 32 outputs
                if i != 0:
                    weights['embed_wc%d' % (i + 1)] = init_weights(
                        [filter_sizes[i], filter_sizes[i], fan_in, num_filters[i]], name='embed_wc%d' % (i + 1))
                else:
                    weights['embed_wc%d' % (i + 1)] = init_weights(
                        [filter_sizes[i], filter_sizes[i], fan_in, num_filters[i]], name='embed_wc%d' % (i + 1))
            ...
            weights['embed_bc%d' % (i + 1)] = init_bias([num_filters[i]], name='embed_bc%d' % (i + 1))
            print(i, weights['embed_wc%d' % (i + 1)])
    ...
    print('construct weights', weights)
...
def construct_goal_input(nn_input, network_config=None):
    """ Preprocess images. """
    ...
    im_width = network_config['image_width']
    num_channels = network_config['image_channels']
    image_input = tf.reshape(nn_input, [-1, num_channels * 2, im_width, im_height])
    image_input = tf.transpose(image_input, perm=[0, 3, 2, 1])
    if FLAGS.pretrain_weight_path != 'N/A':
        image_input = image_input * 255.0 - tf.convert_to_tensor(np.array([103.939, 116.779, 123.68], np.float32))
        # 'RGB'->'BGR'
...
def embed_fc_forward(fc_input, weights, meta_testing=False, is_training=True, testing=False, norm_type='layer_norm',
                     network_config=None, activation_fn=tf.nn.relu):
    n_layers = network_config.get('n_layers', 4)
    use_dropout = FLAGS.dropout
    prob = FLAGS.keep_prob
    fc_output = tf.add(fc_input, 0)
    print('calling embed_fc_forward,input is', fc_input)
    ...
    for i in xrange(n_layers):
        ...
        if (i == n_layers - 1 or (
            ...
            if i == n_layers - 1 and FLAGS.temporal_conv_2_head:
                print(i, 'before reshape embed fc_output', fc_output.get_shape().dims[-1].value)
                # fc_output = tf.reshape(fc_output, [-1, FLAGS.T, fc_output.get_shape().dims[-1].value])
                fc_output = tf.reshape(fc_output, [-1, 1, fc_output.get_shape().dims[-1].value])
                print(i, 'reshape embed fc_output', fc_output.shape)
                strides = network_config.get('strides', [[1, 2, 2, 1], [1, 2, 2, 1], [1, 2, 2, 1]])
                ...
                for j in xrange(temporal_num_layers):
                    if j != temporal_num_layers - 1:
                        fc_output = norm(conv1d(img=fc_output, w=weights['embed_w_1d_conv_2_head_%d' % j],
                                                b=weights['embed_b_1d_conv_2_head_%d' % j]), \
                                         norm_type=norm_type, id=n_conv_layers + j, is_training=is_training,
                                         activation_fn=activation_fn)
                    else:
                        fc_output = conv1d(img=fc_output, w=weights['embed_w_1d_conv_2_head_%d' % j],
                                           b=weights['embed_b_1d_conv_2_head_%d' % j])
                    print(j, 'embed temporal', fc_output.shape, weights['embed_w_1d_conv_2_head_%d' % j])
            else:
                # print('weights are',weights['w_%d_two_heads' % i],weights['b_%d_two_heads' % i])
                print(i, 'embed fc_output', fc_output.shape, weights['embed_w_%d_two_heads' % i])
                fc_output = tf.matmul(fc_output, weights['embed_w_%d_two_heads' % i]) + weights[
                    'embed_b_%d_two_heads' % i]
        elif i == 0 and FLAGS.two_arms:

            if not meta_testing:
                fc_output = tf.matmul(fc_output, weights['embed_w_%d_img' % i]) + weights['embed_b_%d_img' % i]
            else:
                fc_output = tf.matmul(fc_output, weights['embed_w_%d_img' % i]) + weights['embed_b_%d_img' % i]
        else:
            print(i, 'now test', fc_output, weights['embed_w_%d' % i])
            fc_output = tf.matmul(fc_output, weights['embed_w_%d' % i]) + weights['embed_b_%d' % i]

        if i != n_layers - 1:
            fc_output = activation_fn(fc_output)
            # only use dropout for post-update
            if use_dropout:
                fc_output = dropout(fc_output, keep_prob=prob, is_training=is_training, name='embed_dropout_fc_%d' % i,
                                    selu=use_selu)
    # fc_output = tf.reshape(fc_output, [-1, FLAGS.embed_size])
    # fc_output = tf.tile(fc_output, [FLAGS.T, 1])
    print('embed final output', fc_output)

    return fc_output


def fc_forward(fc_input, embed_input, weights, state_input=None, meta_testing=False, is_training=True, testing=False,
    ...
    print('calling fc_forward', n_layers)
    for i in xrange(n_layers):
        if i > 0 and FLAGS.all_fc_bt:
        ...
        if (i == n_layers - 1 or (
                ...
                for j in xrange(temporal_num_layers):
                    if j != temporal_num_layers - 1:
                        ...
                        fc_output = conv1d(img=fc_output, w=weights['w_1d_conv_2_head_%d' % j],
                                           b=weights['b_1d_conv_2_head_%d' % j])
                    print(j, 'fc temporal', fc_output, weights['w_1d_conv_2_head_%d' % j])
            else:
                # print('weights are',weights['w_%d_two_heads' % i],weights['b_%d_two_heads' % i])
            ...
            print(i, 'forward fc_weights', fc_output, embed_input, weights['w_%d' % i])
            if i == 1:
                fc_output = tf.concat(axis=1, values=[fc_output, embed_input])
                print(i, 'calling fc_forward', fc_output.shape, embed_input.shape)
...
def embed_forward(conv_out_size, image_input, weights, meta_testing=False, is_training=True, testing=False,
    ...
    if FLAGS.fc_bt:
        ...
        im_width = network_config['image_width']
        num_channels = network_config['image_channels']
        flatten_image = tf.reshape(image_input, [-1, 2 * im_height * im_width * num_channels])
    ...
    # print('conv_layer is',conv_layer)
    if FLAGS.conv_bt:
        img_context = tf.zeros_like(conv_layer)
        img_context += weights['img_context']
        print('calling FLAGS.conv_bt')
        conv_layer = tf.concat(axis=3, values=[conv_layer, img_context])
    for i in xrange(n_conv_layers):

        if not use_dropout:
            conv_layer = norm(
                conv2d(img=conv_layer, w=weights['embed_wc%d' % (i + 1)], b=weights['embed_bc%d' % (i + 1)],
                       strides=strides[i],
        ...
        print('n_conv_layers:', i, conv_layer, weights['embed_wc%d' % (i + 1)])
    ...
    print('embed_ conv_out_flat', conv_out_flat)
    ...
    print('embed fc_input', fc_input.shape)
    return embed_fc_forward(fc_input, weights, meta_testing=meta_testing, is_training=is_training, testing=testing,
                            norm_type=norm_type, network_config=network_config), final_eept_pred


def forward(conv_out_size, image_input, embed_input, state_input, weights, meta_testing=False, is_training=True,
            testing=False,
            norm_type='layer_norm', network_config=None, activation_fn=tf.nn.relu):
    """ Perform the forward pass. """

    print('calling forward, image_input is', image_input)
    if FLAGS.fc_bt:
        im_height = network_config['image_height']
        im_width = network_config['image_width']
        num_channels = network_config['image_channels']
    ...
    for i in xrange(n_conv_layers):
        ...
        if not use_dropout:
            ...
            conv_layer = dropout(norm(
                ...
                norm_type=norm_type, decay=decay, id=i, is_training=is_training, activation_fn=activation_fn),
                keep_prob=prob, is_training=is_training, name='dropout_%d' % (i + 1))
        print('n_conv_layers:', i, conv_layer, weights['wc%d' % (i + 1)])
    ...
    if FLAGS.fp:
        ...
        print('fp_x, fp_y', fp_x, fp_y)

        conv_out_flat = tf.reshape(tf.concat(axis=1, values=[fp_x, fp_y]), [-1, num_fp * 2])
    else:
        conv_out_flat = tf.reshape(conv_layer, [-1, conv_out_size])

    print('conv_out_flat', conv_out_flat)
    fc_input = tf.add(conv_out_flat, 0)
    if FLAGS.learn_final_eept:
        final_eept_range = range(FLAGS.final_eept_min, FLAGS.final_eept_max)
        if testing:
    ...
    if FLAGS.fc_bt:
        fc_input = tf.concat(axis=1, values=[fc_input, context])
        # print('context test',context.shape)
    return fc_forward(fc_input, embed_input, weights, state_input=state_input, meta_testing=meta_testing,
...
def main():
    ...
    with graph.as_default():
        ...
        global_compare_obsa = tf.placeholder(tf.float32, name='compare_obsa')
        global_compare_obsb = tf.placeholder(tf.float32, name='compare_obsb')
        global_compare_obsc = tf.placeholder(tf.float32, name='compare_obsc')
        ...
        with tf.variable_scope('model', reuse=tf.AUTO_REUSE) as training_scope:
            weights, conv_out_size = construct_weights(data_generator._dU, img_idx, state_idx, FLAGS.norm,
                                                       network_config)
            ...
            def batch_metalearn(inp):
                ...
                inputa, inputb, compare_inputa, compare_inputb, compare_inputc, actiona, actionb = inp
                inputa = tf.reshape(inputa, [-1, dim_input])
                inputb = tf.reshape(inputb, [-1, dim_input])
                compare_inputa = tf.reshape(compare_inputa, [-1, dim_input])
                compare_inputb = tf.reshape(compare_inputb, [-1, dim_input])
                ...
                compare_inputa = construct_goal_input(compare_inputa, network_config=network_config)
                compare_inputb = construct_goal_input(compare_inputb, network_config=network_config)
                compare_inputc = construct_goal_input(compare_inputc, network_config=network_config)
                print('compare_inputa.shape', compare_inputa.shape)

                inputa, _, state_inputa = construct_image_input(inputa, state_idx, img_idx,
                                                                network_config=network_config)
                inputb, _, state_inputb = construct_image_input(inputb, state_idx, img_idx,
                                                                network_config=network_config)
                ...
                if 'Training' in prefix:
                    embed_outputa, _ = embed_forward(conv_out_size, compare_inputa, weights, network_config=network_config)
                else:
                    embed_outputa, _ = embed_forward(conv_out_size, compare_inputa, weights, is_training=False, testing=testing, network_config=network_config)



                if 'Training' in prefix:
                    embed_outputb, _ = embed_forward(conv_out_size, compare_inputb, weights, network_config=network_config)
                else:
                    embed_outputb, _ = embed_forward(conv_out_size, compare_inputb, weights, is_training=False, testing=testing, network_config=network_config)

                if 'Training' in prefix:
                    embed_outputc, _ = embed_forward(conv_out_size, compare_inputc, weights, network_config=network_config)
                else:
                    embed_outputc, _ = embed_forward(conv_out_size, compare_inputc, weights, is_training=False, testing=testing, network_config=network_config)

                pre_embed_loss = tf.abs(FLAGS.margin - euclidean_loss_layer(embed_outputa, embed_outputb, multiplier=loss_multiplier, use_l1=FLAGS.use_l1_l2_loss) +\
                                        euclidean_loss_layer(embed_outputa, embed_outputc, multiplier=loss_multiplier, use_l1=FLAGS.use_l1_l2_loss))

                # embed_outputa = embed_outputa / tf.sqrt(tf.reduce_sum(tf.pow(embed_outputa, 2)))

                # if 'Training' in prefix:
                #     embed_outputa, _ = embed_forward(conv_out_size, compare_inputa, weights, meta_testing=True,  network_config=network_config)
                # else:
                #     embed_outputa, _ = embed_forward(conv_out_size, compare_inputa, weights, meta_testing=True,  is_training=False, testing=testing, network_config=network_config)

                embed_outputa = tf.reshape(embed_outputa, [-1, FLAGS.embed_size])
                embed_outputa = tf.tile(embed_outputa, [FLAGS.T, 1])
                print('embed_outputa.shape', embed_outputa.shape)

                # Pre-update
                if 'Training' in prefix:
                    local_outputa, final_eept_preda = forward(conv_out_size, inputa, embed_outputa,
                                                              tf.zeros_like(state_inputa), weights,
                                                              network_config=network_config)
                else:
                    local_outputa, final_eept_preda = forward(conv_out_size, inputa, embed_outputa,
                                                              tf.zeros_like(state_inputa), weights, is_training=False,
                                                              network_config=network_config)

                local_lossa = act_loss_eps * euclidean_loss_layer(local_outputa, tf.zeros_like(local_outputa),
                ...
                # embedding
                if 'Training' in prefix:
                    embed_outputa, _ = embed_forward(conv_out_size, compare_inputa, fast_weights, meta_testing=True,  network_config=network_config)
                else:
                    embed_outputa, _ = embed_forward(conv_out_size, compare_inputa, fast_weights, meta_testing=True,  is_training=False, testing=testing, network_config=network_config)


                if 'Training' in prefix:
                    embed_outputb, _ = embed_forward(conv_out_size, compare_inputb, fast_weights, meta_testing=True,  network_config=network_config)
                else:
                    embed_outputb, _ = embed_forward(conv_out_size, compare_inputb, fast_weights, meta_testing=True,  is_training=False, testing=testing, network_config=network_config)

                if 'Training' in prefix:
                    embed_outputc, _ = embed_forward(conv_out_size, compare_inputc, fast_weights, meta_testing=True,  network_config=network_config)
                else:
                    embed_outputc, _ = embed_forward(conv_out_size, compare_inputc, fast_weights, meta_testing=True,  is_training=False, testing=testing, network_config=network_config)

                f_embed_loss =  euclidean_loss_layer(embed_outputa, embed_outputb, multiplier=loss_multiplier, use_l1=FLAGS.use_l1_l2_loss)
                t_embed_loss =  euclidean_loss_layer(embed_outputa, embed_outputc, multiplier=loss_multiplier, use_l1=FLAGS.use_l1_l2_loss)

                f_embed_loss = f_embed_loss / (tf.sqrt(tf.reduce_sum(tf.pow(f_embed_loss,2)))+1e-6)
                t_embed_loss = t_embed_loss / (tf.sqrt(tf.reduce_sum(tf.pow(t_embed_loss, 2)))+1e-6)

                # f_embed_loss = 1000 * euclidean_loss_layer(embed_outputa, embed_outputb, multiplier=loss_multiplier, use_l1=FLAGS.use_l1_l2_loss)
                # t_embed_loss = 1000 * euclidean_loss_layer(embed_outputa, embed_outputc, multiplier=loss_multiplier, use_l1=FLAGS.use_l1_l2_loss)

                post_embed_loss = tf.abs(FLAGS.margin - f_embed_loss + t_embed_loss)
                # post_embed_loss =  tf.abs(FLAGS.margin - f_embed_loss + t_embed_loss)
                # embed_outputa = embed_outputa / tf.sqrt(tf.reduce_sum(tf.pow(embed_outputa, 2)))
                embed_outputa = tf.reshape(embed_outputa, [-1, FLAGS.embed_size])
                if FLAGS.train:
                    ...
                    embed_outputa = tf.tile(embed_outputa, [1, 1])
                print('embed_outputa.shape', embed_outputa.shape)

                # Post-update
                if 'Training' in prefix:
                    outputb, final_eept_predb = forward(conv_out_size, inputb, embed_outputa, state_inputb,
                                                        fast_weights, meta_testing=True, network_config=network_config)
                else:
                    outputb, final_eept_predb = forward(conv_out_size, inputb, embed_outputa, state_inputb,
                                                        fast_weights, meta_testing=True, is_training=False,
                                                        testing=testing, network_config=network_config)
                ...
                local_lossb = act_loss_eps * euclidean_loss_layer(outputb, actionb, multiplier=loss_multiplier,
                                                                  use_l1=FLAGS.use_l1_l2_loss)
                ...
                local_fn_output = [local_outputa, local_outputbs, local_outputbs[-1], local_lossa, local_lossesb,
                                   pre_embed_loss, post_embed_loss, f_embed_loss , t_embed_loss]
        ...
        out_dtype = [tf.float32, [tf.float32] * FLAGS.num_updates, tf.float32, tf.float32,
                     [tf.float32] * FLAGS.num_updates, tf.float32, tf.float32, tf.float32, tf.float32]
        result = tf.map_fn(batch_metalearn, elems=(
        global_inputa, global_inputb, global_compare_obsa, global_compare_obsb, global_compare_obsc, global_actiona, global_actionb), dtype=out_dtype)

        # outputas, outputbs, test_output, lossesa, lossesb, final_eept_lossesb, flat_img_inputb, gradients = result
        outputas, outputbs, test_output, lossesa, lossesb, pre_embed_loss, post_embed_loss, f_embed_loss , t_embed_loss = result
        # trainable_vars = tf.trainable_variables()
        total_loss1 = tf.reduce_sum(lossesa) / tf.to_float(FLAGS.meta_batch_size)

        total_pre_embed_loss = tf.reduce_sum(pre_embed_loss) / tf.to_float(FLAGS.meta_batch_size)
        total_post_embed_loss = tf.reduce_sum(post_embed_loss) / tf.to_float(FLAGS.meta_batch_size)

        total_f_embed_loss = tf.reduce_sum(f_embed_loss) / tf.to_float(FLAGS.meta_batch_size)
        total_t_embed_loss = tf.reduce_sum(t_embed_loss) / tf.to_float(FLAGS.meta_batch_size)

        # total_losses2 = [tf.reduce_sum(lossesb[j]) / tf.to_float(FLAGS.meta_batch_size) for j in
        #                  range(FLAGS.num_updates)]

        # total_losses2 = [tf.reduce_sum(lossesb[j]) / tf.to_float(FLAGS.meta_batch_size) for j in
        #                  range(FLAGS.num_updates)] +  total_pre_embed_loss + total_post_embed_loss

        total_losses2 = [tf.reduce_sum(lossesb[j]) / tf.to_float(FLAGS.meta_batch_size) for j in
                         range(FLAGS.num_updates)] +  total_post_embed_loss

        select_scope = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope='select_model')
        train_left_op = tf.train.AdamOptimizer(FLAGS.meta_lr).minimize(total_pre_embed_loss)
        train_right_op = tf.train.AdamOptimizer(FLAGS.meta_lr).minimize(total_post_embed_loss)
        # self_learn_train_op = tf.train.AdamOptimizer(1e-6).minimize(total_post_embed_loss)
        ...
        def train(graph, saver, sess, data_generator, log_dirs, restore_itr=0):
            ...
            for itr in training_range:
                ...
                train_image_datas, train_compare_goal_image_datas, train_goal_image_datas = data_generator.make_compare_batch_data(network_config, restore_iter=itr)
                # print('train_image_datas', train_image_datas.shape, 'train_compare_image_datas', train_compare_goal_image_datas.shape,'train_goal_image_datas',train_goal_image_datas.shape)
                ...
                obsa_compare_goal_input = train_compare_goal_image_datas[:, :FLAGS.update_batch_size, :]
                # obsb_compare_goal_input = train_compare_goal_image_datas[:, FLAGS.update_batch_size:, :]

                obsa_goal_input = train_goal_image_datas[:, :FLAGS.update_batch_size, :]
                obsb_goal_input = train_goal_image_datas[:, FLAGS.update_batch_size:, :]

                # print(itr, 'obsa_input.shape', obsa_input.shape,'obsa_goal_input.shape',obsa_goal_input.shape)
                ...
                feed_dict = {global_obsa: obsa_input,
                             ...
                             global_compare_obsa: obsa_goal_input,
                             global_compare_obsb: obsa_compare_goal_input,
                             global_compare_obsc: obsb_goal_input,
                ...
                input_tensors = [train_op,
                                 total_losses2, total_pre_embed_loss, total_post_embed_loss, total_f_embed_loss, total_t_embed_loss]
                results = sess.run(input_tensors, feed_dict=feed_dict)
                print '(Phase 1)Iteration %d:  average postloss is %.2f ,embed_preloss is %.5f ,embed_postloss is %.5f , f_embed_loss is %.5f , t_embed_loss is %.5f ,' % \
                      (itr, np.mean(results[-5]), np.mean(results[-4]), np.mean(results[-3]), np.mean(results[-2]), np.mean(results[-1]))
                    ...
                    if j in data_generator.demos[i]['demoConditions']:
                        ...
                        for t in range(T):
                            ...
                            feed_dict = {
                                ...
                                global_obsb: obs,
                                global_compare_obsa: compare_selected_demoO,

                                global_statea: selected_demoX.dot(scale) + bias,
                                global_stateb: state.dot(scale) + bias
        ...
        def evaluate_rl_vision_reach(graph, data_generator, sess, exp_string, record_gifs, log_dirs, iterations=10,
                                     learn_examples=150):
                    ...
                    for j in xrange(REACH_DEMO_CONDITIONS):
                        if j in data_generator.demos[i]['demoConditions']:
                            ...
                            if (rl_rewards > -1.5 and rl_rewards > total_reward) or (rl_rewards > -1.0):
                                ...
                                results = sess.run(input_tensors, feed_dict=feed_dict)
                                print 'Self-learnling:Iteration %d_%d_%d: average postloss is %.6f' % (
                                itr, i, j, np.mean(results[-1]))

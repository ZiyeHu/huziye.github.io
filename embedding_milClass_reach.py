...
def construct_fc_weights(dU, img_idx, state_idx, norm_type='layer_norm', dim_input=27, dim_output=7, network_config=None):
    ...
    for i in xrange(n_layers):
        if FLAGS.two_arms and i == 0:
            ...
            if norm_type == 'selu':
                weights['w_%d_img' % i] = init_fc_weights_snn([in_shape - len(state_idx), dim_hidden[i]], name='w_%d_img' % i)
                weights['w_%d_state' % i] = init_fc_weights_snn([len(state_idx), dim_hidden[i]], name='w_%d_state' % i)

                weights['embed_w_%d_img' % i] = init_fc_weights_snn([in_shape - len(state_idx), dim_hidden[i]], name='embed_w_%d_img' % i)
                weights['embed_w_%d_state' % i] = init_fc_weights_snn([len(state_idx), dim_hidden[i]], name='embed_w_%d_state' % i)
            else:
                ...
                weights['embed_w_%d_img' % i] = init_weights([in_shape - len(state_idx), dim_hidden[i]], name='embed_w_%d_img' % i)
                weights['embed_w_%d_state' % i] = init_weights([len(state_idx), dim_hidden[i]], name='embed_w_%d_state' % i)

                weights['b_%d_state_two_arms' % i] = init_bias([dim_hidden[i]], name='b_%d_state_two_arms' % i)
                weights['embed_b_%d_state_two_arms' % i] = init_bias([dim_hidden[i]], name='embed_b_%d_state_two_arms' % i)
        ...
        if norm_type == 'selu':
            ...
            if i<(n_layers-1):
                if i!=0:
                    weights['embed_w_%d' % i] = init_weights([FLAGS.layer_size, FLAGS.layer_size], name='embed_w_%d' % i)
                else:
                    if FLAGS.fp:
                        weights['embed_w_%d' % i] = init_weights([FLAGS.num_filters*2, FLAGS.layer_size], name='embed_w_%d' % i)
                    else:
                        weights['embed_w_%d' % i] = init_weights([FLAGS.num_filters , FLAGS.layer_size], name='embed_w_%d' % i)
            else:
            ...
            if i == n_layers - 1 and FLAGS.temporal_conv_2_head:
                ...
                for j in xrange(len(temporal_num_filters)):
                    if j != len(temporal_num_filters) - 1:
                        ...
                        weights['embed_w_1d_conv_2_head_%d' % j] = init_weights([temporal_kernel_size, in_shape, temporal_num_filters[j]],name='embed_w_1d_conv_2_head_%d' % j)
                        ...
                        weights['embed_w_1d_conv_2_head_%d' % j] = init_weights([1, in_shape, FLAGS.embed_size], name='embed_w_1d_conv_2_head_%d' % j)
                        weights['embed_b_1d_conv_2_head_%d' % j] = init_bias([FLAGS.embed_size],name='embed_b_1d_conv_2_head_%d' % j)


                    print('embed j',j,'temporal', weights['w_1d_conv_2_head_%d' % j], weights['embed_w_1d_conv_2_head_%d' % j])
                ...
                if i==0:
                    if FLAGS.fp:
                        weights['embed_w_%d_two_heads' % i] = init_weights([FLAGS.num_filters * 2, dim_hidden[i]],  name='embed_w_%d_two_heads' % i)
                    else:
                        weights['embed_w_%d_two_heads' % i] = init_weights([FLAGS.num_filters, dim_hidden[i]],name='embed_w_%d_two_heads' % i)
                    weights['embed_b_%d_two_heads' % i] = init_bias([dim_hidden[i]], name='embed_b_%d_two_heads' % i)
                elif i<n_layers - 1:
                    weights['embed_w_%d_two_heads' % i] = init_weights([in_shape, dim_hidden[i]],  name='embed_w_%d_two_heads' % i)
                    weights['embed_b_%d_two_heads' % i] = init_bias([dim_hidden[i]], name='embed_b_%d_two_heads' % i)
    ...
    for i in xrange(n_conv_layers):
        if not pretrain or i != 0:
            ...
            if norm_type == 'selu':
                ...
                if i!=0:
                    weights['embed_wc%d' % (i + 1)] = init_conv_weights_snn([filter_sizes[i], filter_sizes[i], fan_in, num_filters[i]], name='embed_wc%d' % (i + 1))
                else:
                    weights['embed_wc%d' % (i + 1)] = init_conv_weights_snn([filter_sizes[i], filter_sizes[i], 6, num_filters[i]], name='embed_wc%d' % (i + 1))
                ...
                if i != 0:
                    weights['embed_wc%d' % (i + 1)] = init_conv_weights_xavier([filter_sizes[i], filter_sizes[i], fan_in, num_filters[i]],name='embed_wc%d' % (i + 1))
                else:
                    weights['embed_wc%d' % (i + 1)] = init_conv_weights_xavier([filter_sizes[i], filter_sizes[i], 6, num_filters[i]], name='embed_wc%d' % (i + 1))
                ...
                if i != 0:
                    weights['embed_wc%d' % (i + 1)] = init_weights([filter_sizes[i], filter_sizes[i], fan_in, num_filters[i]], name='embed_wc%d' % (i + 1))
                else:
                    weights['embed_wc%d' % (i + 1)] = init_weights([filter_sizes[i], filter_sizes[i], fan_in, num_filters[i]], name='embed_wc%d' % (i + 1))
            else:
...
def embed_fc_forward(fc_input, weights, meta_testing=False, is_training=True, testing=False, norm_type='layer_norm',
    ...
    for i in xrange(n_layers):
            ...
            if i == n_layers - 1 and FLAGS.temporal_conv_2_head:
                ...
                for j in xrange(temporal_num_layers):
                    if j != temporal_num_layers - 1:
                        ...
                        fc_output = conv1d(img=fc_output, w=weights['embed_w_1d_conv_2_head_%d' % j], b=weights['embed_b_1d_conv_2_head_%d' % j])
                ...
                fc_output = tf.matmul(fc_output, weights['embed_w_%d_two_heads' % i]) + weights['embed_b_%d_two_heads' % i]
        ...
        if i != n_layers - 1:
            ...
            if use_dropout:
                fc_output = dropout(fc_output, keep_prob=prob, is_training=is_training, name='embed_dropout_fc_%d' % i, selu=use_selu)
...
def fc_forward(fc_input, embed_input, weights, state_input=None, meta_testing=False, is_training=True, testing=False, norm_type='layer_norm',
               network_config=None, activation_fn=tf.nn.relu):
    n_layers = network_config.get('n_layers', 4)
    use_dropout = FLAGS.dropout
    prob = FLAGS.keep_prob
...
def embed_forward(conv_out_size, image_input, weights, meta_testing=False, is_training=True, testing=False,
    ...
    for i in xrange(n_conv_layers):
        ...
        if not use_dropout:
            conv_layer = norm(
                conv2d(img=conv_layer, w=weights['embed_wc%d' % (i + 1)], b=weights['embed_bc%d' % (i + 1)], strides=strides[i],
    ...
    return embed_fc_forward(fc_input, weights, meta_testing=meta_testing, is_training=is_training, testing=testing, norm_type=norm_type, network_config=network_config), final_eept_pred


def forward(conv_out_size, image_input, embed_input, state_input, weights, meta_testing=False, is_training=True, testing=False,
            norm_type='layer_norm', network_config=None, activation_fn=tf.nn.relu):
    """ Perform the forward pass. """
    ...
    return fc_forward(fc_input, embed_input, weights, state_input=state_input, meta_testing=meta_testing, is_training=is_training,
    ...
    with graph.as_default():
        ...
        with tf.variable_scope('model', reuse=tf.AUTO_REUSE) as training_scope:
            ...
            def batch_metalearn(inp):
                ...
                inputb, _, state_inputb = construct_image_input(inputb, state_idx, img_idx,network_config=network_config)
                ...
                # Pre-update
                if 'Training' in prefix:
                    local_outputa, final_eept_preda = forward(conv_out_size, inputa, embed_outputa, tf.zeros_like(state_inputa), weights, network_config=network_config)
                else:
                    local_outputa, final_eept_preda = forward(conv_out_size, inputa, embed_outputa, tf.zeros_like(state_inputa), weights,  is_training=False, network_config=network_config)
                ...
                # Post-update
                if 'Training' in prefix:
                    outputb, final_eept_predb = forward(conv_out_size, inputb, embed_outputa, state_inputb, fast_weights, meta_testing=True, network_config=network_config)
                else:
                    outputb, final_eept_predb = forward(conv_out_size, inputb, embed_outputa, state_inputb, fast_weights, meta_testing=True, is_training=False, testing=testing, network_config=network_config)
                ...
                local_fn_output = [local_outputa, local_outputbs, local_outputbs[-1], local_lossa, local_lossesb,left_lossb,right_lossb]
        ...
        out_dtype = [tf.float32, [tf.float32] * FLAGS.num_updates, tf.float32, tf.float32,[tf.float32] * FLAGS.num_updates,tf.float32,tf.float32]
        result = tf.map_fn(batch_metalearn, elems=(global_inputa, global_inputb, global_compare_obsa,global_compare_obsb, global_actiona, global_actionb), dtype=out_dtype)
        ...
        def train(graph, saver, sess, data_generator, log_dirs, restore_itr=0):
            ...
            for itr in training_range:
                ...
                train_image_datas,train_compare_image_datas,train_goal_image_datas = data_generator.make_compare_batch_data(network_config, restore_iter=itr)

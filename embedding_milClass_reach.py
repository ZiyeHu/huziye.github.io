    ...
    with graph.as_default():
        ...
        with tf.variable_scope('model', reuse=tf.AUTO_REUSE) as training_scope:
            ...
            def batch_metalearn(inp):
                ...
                if 'Training' in prefix:
                    compare_outputb, compare_final_eept_predb  = forward(conv_out_size, compare_input, compare_state_input, fast_weights,meta_testing=True, network_config=network_config)
                else:
                    compare_outputb, compare_final_eept_predb  = forward(conv_out_size, compare_input, compare_state_input, fast_weights,meta_testing=True, is_training=False, testing=testing,network_config=network_config)

                # semantic_outputb = outputb[:, 2:]
                # outputb = outputb[:,0:2]
                # print('semantic_output',semantic_outputb.shape,'outputb', outputb.shape)

                # semantic_loss = act_loss_eps * euclidean_loss_layer(semantic_outputa, semantic_outputb, multiplier=loss_multiplier,use_l1=FLAGS.use_l1_l2_loss)
        ...
        def train(graph, saver, sess, data_generator, log_dirs, restore_itr=0):
            ...
            for itr in training_range:
                ...
                # print('train_image_datas',train_image_datas.shape, 'train_compare_image_datas',train_compare_image_datas.shape)

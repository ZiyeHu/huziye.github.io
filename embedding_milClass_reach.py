    ...
    with graph.as_default():
        ...
        with tf.variable_scope('model', reuse=tf.AUTO_REUSE) as training_scope:
            ...
            def batch_metalearn(inp):
                ...
                inputa, inputb, compare_inputa, compare_inputb, actiona, actionb = inp
                ...
                pre_embed_loss = tf.abs(1.0 - euclidean_loss_layer(embed_outputa, embed_outputb, multiplier=loss_multiplier, use_l1=FLAGS.use_l1_l2_loss))
                ...
                post_embed_loss = tf.abs(1.0 - euclidean_loss_layer(embed_outputa, embed_outputb, multiplier=loss_multiplier, use_l1=FLAGS.use_l1_l2_loss))
                # embed_outputa = embed_outputa / tf.sqrt(tf.reduce_sum(tf.pow(embed_outputa, 2)))
                embed_outputa = tf.reshape(embed_outputa, [-1, FLAGS.embed_size])
                if FLAGS.train:
                    embed_outputa = tf.tile(embed_outputa, [FLAGS.T, 1])
                ...
                local_fn_output = [local_outputa, local_outputbs, local_outputbs[-1], local_lossa, local_lossesb,
                                   pre_embed_loss, post_embed_loss]
        ...
        out_dtype = [tf.float32, [tf.float32] * FLAGS.num_updates, tf.float32, tf.float32,
                     [tf.float32] * FLAGS.num_updates, tf.float32, tf.float32]
        result = tf.map_fn(batch_metalearn, elems=(
        global_inputa, global_inputb, global_compare_obsa, global_compare_obsb, global_actiona, global_actionb),
                           dtype=out_dtype)

        # outputas, outputbs, test_output, lossesa, lossesb, final_eept_lossesb, flat_img_inputb, gradients = result
        outputas, outputbs, test_output, lossesa, lossesb, pre_embed_loss, post_embed_loss = result
        ...
        def train(graph, saver, sess, data_generator, log_dirs, restore_itr=0):
            ...
            for itr in training_range:
                ...
                # print('train_image_datas', train_image_datas.shape, 'train_compare_image_datas', train_compare_image_datas.shape,'train_goal_image_datas',train_goal_image_datas.shape)
                ...
                input_tensors = [train_op,
                                 total_losses2, total_pre_embed_loss, total_post_embed_loss]
                results = sess.run(input_tensors, feed_dict=feed_dict)
                print '(Phase 1)Iteration %d:  average postloss is %.2f ,embed_preloss is %.5f ,embed_postloss is %.5f ,' % \
